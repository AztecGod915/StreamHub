...
function getPlatformLink(providerName, movieId, movieTitle, tmdbLink) {
  // Primary: TMDB/JustWatch link — this CONFIRMS the title is available
  // and routes user to that exact title on the streaming service.
  // This is the gold standard — JustWatch verifies availability in real time.
  if (tmdbLink) return tmdbLink;

  // Secondary: TMDB direct watch page for this movie ID
  // This still uses JustWatch data and shows all available services
  if (movieId) return `https://www.themoviedb.org/movie/${movieId}/watch?locale=US`;

  // Tertiary: Provider-specific search URLs so user lands with the title pre-searched
  const enc = movieTitle ? encodeURIComponent(movieTitle) : "";
  const searchUrls = {
    "Netflix":              enc ? `https://www.netflix.com/search?q=${enc}` : "https://www.netflix.com",
    "Hulu":                 enc ? `https://www.hulu.com/search?q=${enc}` : "https://www.hulu.com",
    "Disney Plus":          enc ? `https://www.disneyplus.com/search?q=${enc}` : "https://www.disneyplus.com",
    "Disney+":              enc ? `https://www.disneyplus.com/search?q=${enc}` : "https://www.disneyplus.com",
    "Max":                  enc ? `https://www.max.com/search/${enc}` : "https://www.max.com",
    "HBO Max":              enc ? `https://www.max.com/search/${enc}` : "https://www.max.com",
    "Apple TV Plus":        enc ? `https://tv.apple.com/search?term=${enc}` : "https://tv.apple.com",
    "Apple TV+":            enc ? `https://tv.apple.com/search?term=${enc}` : "https://tv.apple.com",
    "Amazon Prime Video":   enc ? `https://www.amazon.com/s?k=${enc}&i=instant-video` : "https://www.amazon.com/video",
    "Prime Video":          enc ? `https://www.amazon.com/s?k=${enc}&i=instant-video` : "https://www.amazon.com/video",
    "Peacock":              enc ? `https://www.peacocktv.com/stream/movies?q=${enc}` : "https://www.peacocktv.com",
    "Peacock Premium":      enc ? `https://www.peacocktv.com/stream/movies?q=${enc}` : "https://www.peacocktv.com",
    "Paramount Plus":       enc ? `https://www.paramountplus.com/search?q=${enc}` : "https://www.paramountplus.com",
    "Paramount+":           enc ? `https://www.paramountplus.com/search?q=${enc}` : "https://www.paramountplus.com",
    "Tubi":                 enc ? `https://tubitv.com/search/${enc}` : "https://tubitv.com",
    "Crunchyroll":          enc ? `https://www.crunchyroll.com/search?q=${enc}` : "https://www.crunchyroll.com",
    "ESPN Plus":            "https://www.espnplus.com",
    "YouTube":              enc ? `https://www.youtube.com/results?search_query=${enc}` : "https://www.youtube.com",
    "Fubo":                 "https://www.fubo.tv",
    "Showtime":             enc ? `https://www.sho.com/search?q=${enc}` : "https://www.sho.com",
    "STARZ":                enc ? `https://www.starz.com/search?q=${enc}` : "https://www.starz.com",
    "MGM Plus":             "https://www.mgmplus.com",
    "Shudder":              enc ? `https://www.shudder.com/search?q=${enc}` : "https://www.shudder.com",
    "Mubi":                 enc ? `https://mubi.com/search/${enc}` : "https://mubi.com",
    "Kanopy":               "https://www.kanopy.com",
    "Hoopla":               "https://www.hoopladigital.com",
  };
  return searchUrls[providerName] || `https://www.themoviedb.org/search?query=${enc}`;
}
...
// Prevents credit drain from repeated/automated calls
const AI_RATE_LIMITS = {
  "mood_search":    { key:"sh_mood_ts",     cooldown:10000  }, // 10s between searches
  "watch_tonight":  { key:"sh_watch_ts",    cooldown:60000  }, // 1 min
  "cost_report":    { key:"sh_cost_ts",     cooldown:300000 }, // 5 mins
  "personalized":   { key:"sh_recs_ts",     cooldown:300000 }, // 5 mins
};
function checkRateLimit(feature) {
  const cfg = AI_RATE_LIMITS[feature];
  if (!cfg) return true;
  const last = parseInt(localStorage.getItem(cfg.key)||"0");
  if (Date.now() - last < cfg.cooldown) return false;
...
const GlobalStyles = () => {
  useEffect(() => {
    ...
    style.textContent = `
      @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Syne:wght@400;600;700;800&family=Plus+Jakarta+Sans:wght@300;400;500;600&display=swap');
      *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
      ...
      @keyframes wcFloat { 0%,100%{transform:translateY(0) rotate(0deg)} 33%{transform:translateY(-10px) rotate(130deg)} 66%{transform:translateY(5px) rotate(260deg)} }
      @keyframes wcFlags { from{transform:translateX(0)} to{transform:translateX(-50%)} }
...
const ESPN_SPORT_MAP = {
  ...
  "wwe":            { path:"football/nfl",             display:"WWE",                  icon:"💪" },
  "wrestling":      { path:"football/nfl",             display:"WWE",                  icon:"💪" },
...
// ─── SPORTS BUZZ TICKER ──────────────────────────────────────────────────────
const BUZZ_FALLBACK = [
  {text:"FIFA World Cup 2026 — Group Stage live scores across USA, Canada & Mexico",type:"breaking",emoji:"⚽",url:null},
  {text:"NBA Free Agency underway — trades and signings all summer long",type:"trending",emoji:"🏀",url:null},
  {text:"MLB Mid-season — standings, scores and highlights daily",type:"news",emoji:"⚾",url:null},
  {text:"Predict match winners and build your streak in the Sports Hub",type:"news",emoji:"🔮",url:null},
  {text:"World Cup 2026 runs through July 19 — every game in the Sports Hub",type:"trending",emoji:"🏆",url:null},
  {text:"UFC Summer cards — fight schedules and where to watch every bout",type:"news",emoji:"🥊",url:null},
  {text:"EPL transfer window heating up — follow your club in the Sports Hub",type:"trending",emoji:"⚽",url:null},
  {text:"NHL offseason moves — trades, signings and draft picks",type:"news",emoji:"🏒",url:null},
  {text:"WNBA season in full swing — live scores and standings",type:"news",emoji:"🏀",url:null},
  {text:"Try Mood Search — tell the AI your vibe, it finds what to watch",type:"news",emoji:"🎭",url:null},
  // Betting odds items
  {text:"ODDS: World Cup Final — Brazil -130 · Argentina +110 · France +140",type:"odds",emoji:"📊",url:"https://www.google.com/search?q=World+Cup+2026+final+odds"},
  {text:"ODDS: NBA MVP Race — SGA -200 · Wembanyama +180 · Tatum +400",type:"odds",emoji:"📊",url:"https://www.espn.com/nba/odds"},
  {text:"ODDS: UFC — Heavyweight title fight — Aspinall -165 · Jones +140",type:"odds",emoji:"📊",url:"https://www.google.com/search?q=UFC+heavyweight+title+odds"},
  {text:"ODDS: MLB World Series — Dodgers +320 · Yankees +450 · Braves +600",type:"odds",emoji:"📊",url:"https://www.espn.com/mlb/odds"},
  {text:"ODDS: EPL Golden Boot — Haaland -150 · Salah +200 · Palmer +350",type:"odds",emoji:"📊",url:"https://www.google.com/search?q=EPL+golden+boot+odds+2026"},
  {text:"ODDS: World Cup Top Scorer — Mbappé +250 · Vinicius +300 · Ronaldo +500",type:"odds",emoji:"📊",url:"https://www.google.com/search?q=World+Cup+2026+top+scorer+odds"},
  {text:"ODDS: NFL MVP 2026 season — Lamar Jackson -180 · C.J. Stroud +300",type:"odds",emoji:"📊",url:"https://www.espn.com/nfl/odds"},
  {text:"ODDS: WNBA Finals — New York Liberty -140 · Las Vegas Aces +120",type:"odds",emoji:"📊",url:"https://www.espn.com/wnba/odds"},
];

const BUZZ_SOURCES = [
  {path:"soccer/fifa.world",label:"World Cup",emoji:"⚽"},
  {path:"football/nfl",    label:"NFL",      emoji:"🏈"},
  {path:"basketball/nba",  label:"NBA",      emoji:"🏀"},
  {path:"baseball/mlb",    label:"MLB",      emoji:"⚾"},
  {path:"hockey/nhl",      label:"NHL",      emoji:"🏒"},
  {path:"soccer/eng.1",    label:"EPL",      emoji:"⚽"},
  {path:"mma/ufc",         label:"UFC",      emoji:"🥊"},
  {path:"basketball/wnba", label:"WNBA",     emoji:"🏀"},
];

const BUZZ_CACHE = "sh_sports_buzz_v9";

// Interleave news + odds so odds appear early and throughout the strip
function interleaveItems(news, odds) {
  const out = [];
  const maxLen = Math.max(news.length, odds.length);
  for (let i = 0; i < maxLen; i++) {
    if (i < news.length) out.push(news[i]);
    if (i < odds.length) out.push(odds[i]);
  }
  return out;
}

const NEWS_FALLBACK = BUZZ_FALLBACK.filter(i=>i.type!=="odds");
const ODDS_FALLBACK = BUZZ_FALLBACK.filter(i=>i.type==="odds");
const INTERLEAVED_FALLBACK = interleaveItems(NEWS_FALLBACK, ODDS_FALLBACK);

function SportsNewsTicker() {
  const [items, setItems] = useState(INTERLEAVED_FALLBACK);
  const [paused, setPaused] = useState(false);

  const loadBuzz = async (force=false) => {
    if (!force) {
      try {
        const c = JSON.parse(localStorage.getItem(BUZZ_CACHE)||"{}");
        if (c.items?.length >= 12 && Date.now()-c.ts < 24*60*60*1000) {
          setItems(c.items); return;
        }
      } catch {}
    }
    try {
      const results = await Promise.allSettled(
        BUZZ_SOURCES.map(s =>
          fetch(`https://site.api.espn.com/apis/site/v2/sports/${s.path}/news?limit=4`)
            .then(r=>r.json())
            .then(d=>(d.articles||[]).slice(0,4).map((a,i)=>({
              text:  a.headline||"",
              url:   a.links?.web?.href||null,
              emoji: s.emoji,
              type:  i===0 ? "breaking" : i<=1 ? "trending" : "news",
            })).filter(a=>a.text))
        )
      );
      const live = results.filter(r=>r.status==="fulfilled").flatMap(r=>r.value);
      if (live.length >= 4) {
        const merged = interleaveItems(live, ODDS_FALLBACK);
        setItems(merged);
        localStorage.setItem(BUZZ_CACHE, JSON.stringify({items:merged, ts:Date.now()}));
      }
    } catch {}
  };
  ...
  useEffect(()=>{
    loadBuzz();
    const iv = setInterval(()=>loadBuzz(true), 24*60*60*1000);
    return ()=>clearInterval(iv);
  },[]);

  const PILL = {
    breaking:{bg:"rgba(239,68,68,.18)", border:"rgba(239,68,68,.55)", badge:"#ef4444", label:"🚨 BREAKING"},
    trending: {bg:"rgba(239,68,68,.18)", border:"rgba(239,68,68,.55)", badge:"#ef4444", label:"🚨 BREAKING"},
    news:     {bg:"rgba(239,68,68,.18)", border:"rgba(239,68,68,.55)", badge:"#ef4444", label:"🚨 BREAKING"},
    odds:     {bg:"rgba(16,185,129,.1)", border:"rgba(16,185,129,.4)",  badge:"#10B981", label:"📊 ODDS"},
  };

  // Split items into two rows — odds on row2, news on row1 (interleaved already so just split evenly)
  const row1items = items.filter((_,i)=>i%2===0);
  const row2items = items.filter((_,i)=>i%2===1);

  const PillSet = ({suffix, rowItems}) => rowItems.map((h,i) => {
    const p = PILL[h.type] || PILL.news;
    return (
      <span key={`${suffix}-${i}`}
        ...
        style={{
          ...
          borderRadius:99, padding:"1px 7px 1px 2px",
          cursor:h.url?"pointer":"default", flexShrink:0, marginRight:6,
        }}>
        <span style={{
          background:p.badge, color:"#fff", fontSize:7, fontWeight:900,
          letterSpacing:.4, borderRadius:99, padding:"1px 4px",
          flexShrink:0, whiteSpace:"nowrap",
        }}>{p.label}</span>
        <span style={{fontSize:10, fontWeight:600, color:"rgba(240,240,250,.95)", whiteSpace:"nowrap"}}>
          {h.emoji} {h.text}
        </span>
      </span>
    );
  ...
  // Hover-to-pause: set hovered state on mouse enter/leave
  const [hovered, setHovered] = useState(false);
  const isAnimPaused = paused || hovered;
  ...
  return (
    <div style={{
      ...
      boxShadow:"0 0 20px rgba(239,68,68,.3), 0 0 40px rgba(239,68,68,.12)",
      display:"flex",
    }}>
      {/* Label — click to pause/play */}
      <div onClick={()=>setPaused(p=>!p)} style={{
        flexShrink:0, width:76,
        background:"rgba(239,68,68,.1)",
        borderRight:"1px solid rgba(239,68,68,.3)",
        boxShadow:"4px 0 18px rgba(239,68,68,.3)",
        color:"#ef4444",
        display:"flex", flexDirection:"column",
        alignItems:"center", justifyContent:"center",
        textAlign:"center", cursor:"pointer",
        gap:2, padding:"4px", textTransform:"uppercase",
      }}>
        <span style={{fontSize:9,fontWeight:900,letterSpacing:.8,lineHeight:1.2,textShadow:"0 0 12px rgba(239,68,68,.9)"}}>🚨 SPORTS</span>
        <span style={{fontSize:9,fontWeight:900,letterSpacing:.8,lineHeight:1.2,textShadow:"0 0 12px rgba(239,68,68,.9)"}}>NEWS &amp;</span>
        <span style={{fontSize:9,fontWeight:900,letterSpacing:.8,lineHeight:1.2,textShadow:"0 0 12px rgba(16,185,129,.9)"}}>ODDS</span>
        <span style={{fontSize:13,lineHeight:1,marginTop:2,textShadow:"0 0 10px rgba(239,68,68,1)"}}>{paused?"▶":"⏸"}</span>
      </div>
      ...
      >
        {/* Row 1 */}
        <div style={{height:"50%", overflow:"hidden", display:"flex", alignItems:"center", borderBottom:"1px solid rgba(239,68,68,.1)"}}>
          <div style={{
            display:"inline-flex", alignItems:"center",
            animation:`tickerScroll ${duration}s linear infinite`,
            animationPlayState: isAnimPaused ? "paused" : "running",
            paddingLeft:8,
          }}>
            <PillSet suffix="r1a" rowItems={row1items}/>
            <PillSet suffix="r1b" rowItems={row1items}/>
          </div>
        </div>
        {/* Row 2 — scrolls opposite direction */}
        <div style={{height:"50%", overflow:"hidden", display:"flex", alignItems:"center"}}>
          <div style={{
            display:"inline-flex", alignItems:"center",
            animation:`tickerScrollReverse ${duration * 1.1}s linear infinite`,
            animationPlayState: isAnimPaused ? "paused" : "running",
            paddingLeft:8,
          }}>
            <PillSet suffix="r2a" rowItems={row2items}/>
            <PillSet suffix="r2b" rowItems={row2items}/>
          </div>
        </div>
      </div>
...
function LiveSportsSection({ sportQuery, favoriteTeams, onToggleFavorite, user, showToast, onPredResult, cols=2 }) {
  ...
  const parseEvents = (data) => (data.events||[]).map(evt => {
    ...
    return {
      ...
      periodText,
      home: { name:home?.team?.shortDisplayName||home?.team?.displayName||"", abbr:home?.team?.abbreviation||"", score:home?.score!=null&&home?.score!="" ? home.score : (isLive||st?.completed?"0":""), logo:home?.team?.logo||"", color:home?.team?.color||"333", winner:home?.winner },
      away: { name:away?.team?.shortDisplayName||away?.team?.displayName||"", abbr:away?.team?.abbreviation||"", score:away?.score!=null&&away?.score!="" ? away.score : (isLive||st?.completed?"0":""), logo:away?.team?.logo||"", color:away?.team?.color||"333", winner:away?.winner },
  ...
  const doFetch = async (silent=false, offset=dateOffset) => {
    const sport = sportRef.current;
    if (!sport) return;
    if (!silent) setLoading(true);
    try {
      const dateParam = `?dates=${getDateStr(offset)}`;
      ...
      setIsPolling(offset === 0 && (evts.some(e=>e.isLive) || hasSoonGame));
    } catch(e) {
  ...
  useEffect(() => {
    if (!sportRef.current) return;
    setEvents([]);
    doFetch(false, dateOffset);
  }, [dateOffset]);
  ...
  const recentEvents = events.filter(e=>e.isOver).slice(-6).reverse();
  const hasLive = liveEvents.length > 0;

  const sortByFav = (evts) => {
    if (!favTeam) return evts;
    // Favorite team's games always first, rest after — no cap so live games never get hidden
  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:12}}>
        {[{label:"Yesterday",offset:-1},{label:"Today",offset:0},{label:"Tomorrow",offset:1}].map(({label,offset})=>{
          const active = dateOffset===offset;
          const isToday = offset===0;
          const bg   = active ? (isToday?"rgba(239,68,68,.18)":"rgba(139,92,246,.18)") : "rgba(255,255,255,.03)";
          const bdr  = active ? (isToday?"rgba(239,68,68,.6)":"rgba(139,92,246,.6)")   : "rgba(255,255,255,.07)";
          const clr  = active ? (isToday?"#ff6b6b":"#C4B5FD") : "rgba(240,240,250,.35)";
          const glow = active ? (isToday?"0 0 16px rgba(239,68,68,.35), 0 2px 8px rgba(239,68,68,.2)":"0 0 16px rgba(139,92,246,.35), 0 2px 8px rgba(139,92,246,.2)") : "none";
          return (
            <button key={offset} onClick={()=>setDateOffset(offset)}
              style={{
                flex:1,
                background:bg,
              ...
              {/* Shine strip on active */}
              {active && <div style={{position:"absolute",inset:0,background:`linear-gradient(180deg,${isToday?"rgba(239,68,68,.12)":"rgba(139,92,246,.12)"} 0%,transparent 100%)`,pointerEvents:"none"}}/>}
              {label}
              {active && isToday && <div style={{position:"absolute",bottom:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,#ef4444,transparent)"}}/>}
              {active && !isToday && <div style={{position:"absolute",bottom:0,left:0,right:0,height:2,background:"linear-gradient(90deg,transparent,#8B5CF6,transparent)"}}/>}
            </button>
          );
        })}
      </div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:8}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {hasLive && dateOffset===0 && <div style={{width:8,height:8,borderRadius:"50%",background:"#ef4444",animation:"pulse 1.5s infinite",boxShadow:"0 0 8px #ef4444",flexShrink:0}}/>}
          <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:15,color:hasLive&&dateOffset===0?"#ef4444":"var(--sports)"}}>
            {hasLive&&dateOffset===0?"🔴 LIVE NOW":dateOffset===-1?"📋 YESTERDAY":dateOffset===1?"📅 TOMORROW":"📅 TODAY"} — {sportInfo?.display||""}
          </div>
          {isPolling && dateOffset===0 && <div style={{fontSize:9,background:"rgba(239,68,68,.15)",border:"1px solid rgba(239,68,68,.3)",borderRadius:99,padding:"2px 8px",color:"#ef4444",fontWeight:700}}>AUTO-UPDATING</div>}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {lastUpdated && <div style={{fontSize:13,color:"var(--muted)",fontWeight:600}}>🕐 {lastUpdated.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div>}
          <button onClick={()=>doFetch(false, dateOffset)} style={{background:"rgba(16,185,129,.15)",border:"1.5px solid rgba(16,185,129,.4)",borderRadius:10,color:"var(--sports)",padding:"8px 18px",fontSize:18,fontWeight:900,cursor:"pointer",boxShadow:"0 0 12px rgba(16,185,129,.2)",lineHeight:1}}>↻</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div>
          ...
          {/* Live games always at top with prominent indicator */}
          {liveEvents.length>0 && (
            <>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",background:"#ef4444",boxShadow:"0 0 6px #ef4444",animation:"pulse 1s ease-in-out infinite"}}/>
                <span style={{fontSize:11,color:"#ef4444",letterSpacing:1.2,fontWeight:800}}>LIVE NOW — TAP TO WATCH</span>
              </div>
          ...
          {recentEvents.length>0 && (
            <>
              <div style={{fontSize:11,color:"var(--muted)",letterSpacing:1.2,fontWeight:700,margin:"12px 0 8px"}}>
                {upcomingEvents.length>0||liveEvents.length>0 ? "COMPLETED" : "RECENT RESULTS"}
              </div>
              <div style={{display:"grid",gridTemplateColumns:`repeat(${cols},1fr)`,gap:10}}>
...
const PRED_MILESTONES = [
  {n:1,  icon:"🎯", label:"First Blood",    msg:"You got your first one right! The streak begins.",        color:"#10B981"},
  {n:3,  icon:"🔥", label:"On Fire",        msg:"3 in a row. The AI's scared of you.",                     color:"#F59E0B"},
  {n:5,  icon:"⚡", label:"Electric",       msg:"5 straight correct. Genuinely impressive.",               color:"#8B5CF6"},
  {n:7,  icon:"🧠", label:"Big Brain",      msg:"7 correct. Are you actually watching the scouting reports?", color:"#06B6D4"},
  {n:10, icon:"👑", label:"Oracle",         msg:"10 in a row. You don't watch football. You predict it.",  color:"#F59E0B"},
  {n:15, icon:"🐐", label:"GOAT",           msg:"15 straight. Undisputed. Legendary. We're not worthy.",   color:"#EF4444"},
];
const getPredStats = () => {
  try { return JSON.parse(localStorage.getItem("sh_pred_stats") || '{"streak":0,"best":0,"total":0,"correct":0,"points":0,"badges":[]}'); }
  catch { return {streak:0,best:0,total:0,correct:0,points:0,badges:[]}; }
};
const savePredStats = (s, userId=null) => {
  localStorage.setItem("sh_pred_stats", JSON.stringify(s));
  // Sync to Supabase so stats persist across devices
  if (userId) {
    supabase.from("profiles").update({ pred_stats: JSON.stringify(s) }).eq("id", userId).then(()=>{}).catch?.(()=>{});
  }
};
const getPointsForStreak = n => n>=15?50:n>=10?35:n>=7?25:n>=5?20:n>=3?15:10;
...
function ConfettiBurst({ active }) {
  ...
  const pieces = Array.from({length:30},(_,i)=>({
    ...
    color:["#F59E0B","#8B5CF6","#10B981","#EF4444","#06B6D4","#fff"][Math.floor(Math.random()*6)],
...
function PredictionCelebrationModal({ streak, points, milestone, onClose }) {
  ...
  useEffect(()=>{ 
    const t1=setTimeout(()=>setShowConfetti(false),1500);
    const t2=setTimeout(onClose,5000); 
    return()=>{clearTimeout(t1);clearTimeout(t2);}; 
  },[]);
  const isBig = milestone?.n >= 5;
  const color = milestone?.color || "#10B981";

  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:2500,display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:"rgba(0,0,0,.6)",backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{
        position:"relative",overflow:"hidden",
        background:"var(--surface)",borderRadius:28,padding:"36px 32px 28px",
        border:`2px solid ${color}66`,
        boxShadow:`0 0 80px ${color}33, 0 40px 80px rgba(0,0,0,.6)`,
        textAlign:"center",maxWidth:340,width:"100%",
      }}>
        <ConfettiBurst active={showConfetti}/>

        {/* Big icon */}
        <div style={{fontSize:isBig?80:64,marginBottom:8,lineHeight:1,animation:isBig?"logoFloat 1.5s ease-in-out infinite":"none"}}>
          {milestone?.icon||"✅"}
        </div>

        {/* Badge label */}
        {milestone && (
          <div style={{display:"inline-block",background:`${color}22`,border:`1px solid ${color}66`,borderRadius:99,padding:"4px 14px",fontSize:11,fontWeight:800,color,letterSpacing:1,marginBottom:12}}>
            {milestone.label.toUpperCase()}
          </div>
        )}

        {/* Headline */}
        <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:isBig?26:22,marginBottom:8,
          background:`linear-gradient(135deg,${color},${color}99)`,
          WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1.2}}>
          {milestone?.n===1?"You called it right!":`${streak} in a row.`}
        </div>

        {/* Message */}
        <div style={{fontSize:13,color:"rgba(240,240,250,.55)",marginBottom:24,lineHeight:1.6,minHeight:40}}>
          {milestone?.msg||"You called it right!"}
        </div>

        {/* Stats row */}
        <div style={{display:"flex",gap:10,justifyContent:"center",marginBottom:24}}>
          <div style={{background:`${color}15`,border:`1px solid ${color}44`,borderRadius:14,padding:"12px 20px",flex:1}}>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:26,color}}>+{points}</div>
            <div style={{fontSize:9,color:"var(--muted)",fontWeight:800,letterSpacing:1}}>POINTS</div>
          </div>
          <div style={{background:"rgba(245,158,11,.1)",border:"1px solid rgba(245,158,11,.3)",borderRadius:14,padding:"12px 20px",flex:1}}>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:26,color:"#F59E0B"}}>{streak}🔥</div>
            <div style={{fontSize:9,color:"var(--muted)",fontWeight:800,letterSpacing:1}}>STREAK</div>
          </div>
        </div>

        {/* Next milestone teaser */}
        {(()=>{
          const next = PRED_MILESTONES.find(m=>m.n > streak);
          if (!next) return null;
          const needed = next.n - streak;
          return (
            <div style={{background:"rgba(255,255,255,.03)",borderRadius:10,padding:"8px 14px",marginBottom:16,fontSize:11,color:"rgba(240,240,250,.4)"}}>
              {needed===1?"One more correct pick to unlock ":""}
              {needed>1?`${needed} more to unlock `:""}
              <span style={{color:next.color,fontWeight:700}}>{next.icon} {next.label}</span>
            </div>
          );
        })()}
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center"}}>
          <button onClick={()=>{
            ...
            const text=`🔮 I just called it on The StreamHub!\n\n🔥 ${s.streak} correct in a row · 🏆 Best streak: ${s.best}\n\nCan you beat my prediction streak? → thestreamhub.app`;
            navigator.share?navigator.share({title:"My StreamHub Prediction",text,url:"https://thestreamhub.app"}).catch(()=>{}):navigator.clipboard?.writeText(text).then(()=>{});
          }} style={{background:"rgba(139,92,246,.15)",border:"1px solid rgba(139,92,246,.4)",borderRadius:12,color:"#C4B5FD",padding:"10px 20px",fontSize:13,cursor:"pointer",fontWeight:700}}>
            📤 Share
          </button>
          <button onClick={onClose} style={{background:"rgba(255,255,255,.07)",border:"none",borderRadius:12,color:"rgba(240,240,250,.6)",padding:"10px 20px",fontSize:13,cursor:"pointer",fontWeight:700}}>
            Keep predicting →
          </button>
        </div>
      </div>
...
// Wrong pick feedback modal
function PredictionMissModal({ onClose, streak }) {
  useEffect(()=>{ const t=setTimeout(onClose,3500); return()=>clearTimeout(t); },[]);
  const msgs = ["Better luck next game 😅","You'll get the next one 💪","Every Oracle misses sometimes 🔮","The streak starts now 🔥","Nobody bats 1000 📊"];
  const msg = msgs[Math.floor(Math.random()*msgs.length)];
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:2500,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:20,pointerEvents:"all"}}>
      <div onClick={e=>e.stopPropagation()} className="fadeUp" style={{
        background:"var(--surface)",borderRadius:20,padding:"20px 24px",
        border:"1px solid rgba(239,68,68,.3)",
        boxShadow:"0 0 40px rgba(239,68,68,.12)",
        textAlign:"center",width:"100%",maxWidth:360,marginBottom:80,
      }}>
        <div style={{fontSize:36,marginBottom:8}}>😤</div>
        <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:18,color:"#EF4444",marginBottom:4}}>Wrong call</div>
        <div style={{fontSize:13,color:"rgba(240,240,250,.5)",marginBottom:4}}>{msg}</div>
        {streak>0&&<div style={{fontSize:11,color:"rgba(239,68,68,.6)"}}>Streak of {streak} broken</div>}
      </div>
    </div>
  );
}

function PredictionStatsBar({ compact=false }) {
  const s = getPredStats();
  const acc = s.total > 0 ? Math.round(s.correct/s.total*100) : 0;
  const milestone = [...PRED_MILESTONES].reverse().find(m=>s.streak>=m.n);
  const nextMilestone = PRED_MILESTONES.find(m=>m.n > s.streak);
  const progressPct = nextMilestone ? Math.round((s.streak/(nextMilestone.n))*100) : 100;

  const sharePredictions = () => {
    const text = s.total > 0
      ? `🔮 I'm on a ${s.streak}-game prediction streak on The StreamHub!\n\n⚡ ${s.streak} correct in a row · 🏆 Best: ${s.best} · 📊 ${s.correct}/${s.total} (${acc}% accuracy)${milestone?`\n\n${milestone.icon} ${milestone.label} badge unlocked!`:""}\n\nThink you can beat my streak? Try predictions at thestreamhub.app 👀`
      : `🔮 I just started predicting sports games on The StreamHub!\n\nLive World Cup scores + pick who wins every match. Can you get a longer streak than me?\n\nTry it free → thestreamhub.app`;
    navigator.share
      ? navigator.share({title:"My StreamHub Predictions", text, url:"https://thestreamhub.app"}).catch(()=>{})
  ...
  if (s.total === 0) return (
    <div style={{
      ...
      borderRadius:16, padding:compact?"8px 10px":"14px 14px",
      display:"flex", flexDirection:"column", gap:compact?4:10,
      cursor:"default", minHeight:compact?0:120,
      height:compact?"100%":"auto", boxSizing:"border-box",
    }}>
      <div style={{fontSize:compact?18:40,lineHeight:1,textAlign:"center"}}>🔮</div>
      <div>
        <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:compact?11:13,color:"#111",marginBottom:2}}>Predictions</div>
        {!compact&&<div style={{fontSize:10,color:"rgba(0,0,0,.5)",lineHeight:1.4}}>Tap any game to pick the winner. Build a streak &amp; earn badges.</div>}
        {compact&&<div style={{fontSize:9,color:"rgba(0,0,0,.45)"}}>Tap a game to pick the winner</div>}
      </div>
      <button onClick={sharePredictions} style={{marginTop:"auto",background:"rgba(0,0,0,.07)",border:"1px solid rgba(0,0,0,.15)",borderRadius:8,color:"#333",padding:"4px 0",fontSize:9,fontWeight:700,cursor:"pointer",width:"100%"}}>📤 Share</button>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      minHeight:compact?0:120, height:compact?"100%":"auto",
      boxSizing:"border-box", minWidth:0, overflow:"hidden",
    }}>
      {/* Title row */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:compact?5:10}}>
        <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:compact?11:13,color:"#111",display:"flex",alignItems:"center",gap:4}}>
          <span style={{fontSize:compact?14:28,marginRight:2}}>🔮</span>Predictions {milestone&&<span style={{fontSize:compact?10:14}}>{milestone.icon}</span>}
        </div>
        <button onClick={sharePredictions} style={{background:"rgba(0,0,0,.07)",border:"1px solid rgba(0,0,0,.15)",borderRadius:7,color:"#333",padding:"2px 6px",fontSize:9,fontWeight:700,cursor:"pointer"}}>📤</button>
      </div>
      {/* Stats — vertical list in compact, 2x2 grid in full */}
      {compact ? (
        <div style={{display:"flex",flexDirection:"column",gap:3,flex:1}}>
          {[
            {val:`${s.streak}🔥`, label:"Streak",  color:"#b45309", bg:"#fef3c7", border:"#f59e0b"},
            {val:`${s.best}⚡`,   label:"Best",    color:"#5b21b6", bg:"#ede9fe", border:"#8b5cf6"},
            {val:`${acc}%`,       label:"Correct", color:"#065f46", bg:"#d1fae5", border:"#10b981"},
            {val:`${s.points}`,   label:"Points",  color:"#92400e", bg:"#fef3c7", border:"#f59e0b"},
          ].map(st=>(
            <div key={st.label} style={{
              ...
              <div style={{fontSize:8,color:st.color,fontWeight:700,opacity:.75,flexShrink:0}}>{st.label}</div>
              <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:12,color:st.color,lineHeight:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginLeft:4}}>{st.val}</div>
            </div>
          ))}
          {/* Share button below pills */}
          <button onClick={sharePredictions} style={{
            width:"100%", marginTop:4,
            background:"rgba(0,0,0,.07)", border:"1.5px solid rgba(0,0,0,.15)",
            borderRadius:8, color:"#333", padding:"6px 0",
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:10}}>
          {[
            ...
            <div key={st.label} style={{background:st.bg,border:`2px solid ${st.border}`,borderRadius:9,padding:"8px 6px",textAlign:"center"}}>
              <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:16,color:st.color,lineHeight:1}}>{st.val}</div>
              <div style={{fontSize:8,color:st.color,opacity:.7,fontWeight:700,letterSpacing:.4,marginTop:3}}>{st.label}</div>
            </div>
          ))}
        </div>
      )}
      {/* Progress bar — hidden on compact to keep card slim */}
      {!compact && nextMilestone ? (
        <div style={{marginTop:"auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
            <div style={{fontSize:9,color:"rgba(0,0,0,.45)"}}>Next: {nextMilestone.icon} {nextMilestone.label}</div>
            <div style={{fontSize:9,color:nextMilestone.color,fontWeight:700}}>{s.streak}/{nextMilestone.n}</div>
          </div>
          <div style={{height:4,background:"rgba(0,0,0,.15)",borderRadius:99,overflow:"hidden"}}>
            <div style={{height:"100%",width:`${progressPct}%`,background:`linear-gradient(90deg,${nextMilestone.color}88,${nextMilestone.color})`,borderRadius:99}}/>
          </div>
        </div>
      ) : !compact ? <div style={{marginTop:"auto",textAlign:"center",fontSize:10,color:"#111",fontWeight:700}}>🐐 GOAT status</div> : null}
      {/* Share with a friend — full size only */}
      {!compact && (
        <button onClick={sharePredictions} style={{
          width:"100%", marginTop:12,
          background:"rgba(0,0,0,.06)", border:"1.5px solid rgba(0,0,0,.15)",
          borderRadius:10, color:"#333", padding:"8px 0",
...
function PredictionPoll({ evt, user, showToast, onResult }) {
  ...
  useEffect(() => {
    ...
    if (predCorrect) {
      ...
      const newBest = Math.max(old.best, newStreak);
      const allBadges = [...(old.badges||[])];
      const milestone = [...PRED_MILESTONES].reverse().find(m=>newStreak===m.n);
      if (milestone && !allBadges.includes(milestone.n)) allBadges.push(milestone.n);
      savePredStats({streak:newStreak,best:newBest,total:old.total+1,correct:old.correct+1,points:old.points+pts,badges:allBadges}, user?.id);
      onResult?.({correct:true, streak:newStreak, points:pts, milestone, gameId:evt.id});
    } else {
      savePredStats({...old, streak:0, total:old.total+1, badges:old.badges||[]}, user?.id);
      onResult?.({correct:false, streak:old.streak, points:0, gameId:evt.id});
    }
  ...
  const makePick = async (pick) => {
    ...
    setSaving(false);
    // Streak tension message
    if (stats.streak >= 3) {
      showToast?.(`🔥 ${stats.streak}-game streak on the line! Let's go.`);
    } else {
      showToast?.("🔮 Locked in! Check back after the final whistle.");
    }
  };

  const teamName = p => p==="home"?evt.home?.name:p==="away"?evt.away?.name:"Draw";
  const teamAbbr = p => p==="home"?evt.home?.abbr||evt.home?.name?.slice(0,3).toUpperCase():p==="away"?evt.away?.abbr||evt.away?.name?.slice(0,3).toUpperCase():"X";

  const shareGamePick = () => {
    const awayName = evt.away?.name||"Away";
    const homeName = evt.home?.name||"Home";
    const pickName = myPick?.pick==="home" ? homeName : myPick?.pick==="away" ? awayName : "a Draw";
    const resultText = predCorrect===true
      ? `✅ Called it! ${pickName} won and I nailed it.`
      : predCorrect===false
      ? `❌ Got that one wrong — but I'm still predicting!`
      : `⏳ Waiting on the result...`;
    const text = `🔮 I predicted ${pickName} to win in ${awayName} vs ${homeName} on The StreamHub!\n\n${resultText}\n\nMake your own predictions → thestreamhub.app`;
    navigator.share
      ? navigator.share({title:"My Game Prediction",text,url:"https://thestreamhub.app"}).catch(()=>{})
  ...
  return (
    <div style={{borderTop:"1px solid rgba(139,92,246,.15)",padding:"10px 10px 11px",background:"rgba(139,92,246,.04)"}}>

      {/* Pre-pick — prediction prompt */}
      {canPredict && (
        <>
          <div style={{fontSize:9,fontWeight:800,color:"#A78BFA",letterSpacing:1.5,marginBottom:8,display:"flex",alignItems:"center",gap:5}}>
            🔮 <span>WHO WINS? TAP TO PREDICT</span>
            {stats.streak>=3&&<span style={{color:"#F59E0B",marginLeft:"auto"}}>🔥{stats.streak} streak</span>}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 40px 1fr",gap:5}}>
            {["home","draw","away"].map(p=>{
              const isHov = hovPick===p;
              const colors = {home:"#8B5CF6",draw:"rgba(255,255,255,.5)",away:"#F59E0B"};
              const bg = {home:"rgba(139,92,246,.15)",draw:"rgba(255,255,255,.06)",away:"rgba(245,158,11,.12)"};
              return (
                <button key={p}
                  onClick={e=>{e.stopPropagation();makePick(p);}}
                  onMouseEnter={()=>setHovPick(p)}
                  onMouseLeave={()=>setHovPick(null)}
                  style={{
                    background:isHov?bg[p]:(p==="draw"?"rgba(255,255,255,.04)":"rgba(255,255,255,.04)"),
                    border:`1.5px solid ${isHov?colors[p]:"rgba(255,255,255,.1)"}`,
                    borderRadius:10,
                    padding:p==="draw"?"7px 2px":"7px 5px",
                    fontSize:p==="draw"?9:10,fontWeight:800,
                    color:isHov?colors[p]:"rgba(240,240,250,.6)",
                    cursor:"pointer",
                    overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",
                    transition:"all .15s",
                    transform:isHov?"scale(1.03)":"scale(1)",
                  }}>
                  {p==="draw"?"Draw":teamAbbr(p)}
                </button>
              );
            })}
          </div>
      ...
      {/* Post-pick, game pending — show community votes + Change Pick button */}
      {myPick && !evt.isOver && (
        <>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
            <div style={{fontSize:9,fontWeight:800,color:"#A78BFA",letterSpacing:1}}>
              🔮 YOUR PICK: <span style={{color:"#C4B5FD"}}>{teamAbbr(myPick.pick)}</span>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              {stats.streak>=3&&<div style={{fontSize:9,color:"#F59E0B",fontWeight:700}}>🔥 {stats.streak} streak</div>}
              {isUpcoming&&<button onClick={e=>{e.stopPropagation();localStorage.removeItem(key);setMyPick(null);setCommunity(null);}} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:6,color:"var(--muted)",fontSize:9,fontWeight:700,padding:"2px 8px",cursor:"pointer"}}>✏️ Change</button>}
              <button onClick={e=>{e.stopPropagation();shareGamePick();}} style={{background:"rgba(139,92,246,.12)",border:"1px solid rgba(139,92,246,.3)",borderRadius:6,color:"#C4B5FD",fontSize:9,fontWeight:700,padding:"2px 8px",cursor:"pointer"}}>📤</button>
            </div>
          </div>
          {community ? (
            <>
              <div style={{display:"grid",gridTemplateColumns:"1fr 40px 1fr",gap:5}}>
                {["home","draw","away"].map(p=>{
                  const isMine = myPick.pick===p;
                  return (
                    <div key={p} style={{
                      background:isMine?"rgba(139,92,246,.18)":"rgba(255,255,255,.04)",
                      border:`1.5px solid ${isMine?"rgba(139,92,246,.5)":"rgba(255,255,255,.07)"}`,
                      borderRadius:9,padding:"6px 3px",textAlign:"center",
                    }}>
                      <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:13,color:isMine?"#C4B5FD":"rgba(240,240,250,.4)"}}>{community[p]}%</div>
                      <div style={{height:2,background:"rgba(255,255,255,.06)",borderRadius:99,margin:"3px 4px"}}>
                        <div style={{height:"100%",width:`${community[p]}%`,background:isMine?"#8B5CF6":"rgba(255,255,255,.15)",borderRadius:99}}/>
                      </div>
                      <div style={{fontSize:8,color:"rgba(240,240,250,.25)",marginTop:2}}>{teamAbbr(p)}</div>
                    </div>
                  );
                })}
              </div>
              <div style={{fontSize:9,color:"rgba(240,240,250,.2)",textAlign:"center",marginTop:5}}>{community.total} StreamHub fan{community.total!==1?"s":""} voted</div>
            </>
          ) : (
            <div style={{fontSize:10,color:"rgba(240,240,250,.3)",textAlign:"center",padding:"6px 0"}}>Waiting for others to vote…</div>
          )}
        </>
      )}
      ...
      {myPick && evt.isOver && predCorrect !== null && (
        <div style={{
          ...
          <div>
            <div style={{fontSize:12,fontWeight:800,color:predCorrect?"#10B981":"#ef4444",marginBottom:2}}>
              {predCorrect?"✅ You called it!":"❌ Wrong call"}
            </div>
            <div style={{fontSize:9,color:"rgba(240,240,250,.3)"}}>
              You: {teamAbbr(myPick.pick)} · Result: {actualResult?teamAbbr(actualResult):"—"}
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            {predCorrect && (()=>{
              ...
              return m?<div style={{textAlign:"center"}}><div style={{fontSize:22}}>{m.icon}</div><div style={{fontSize:8,fontWeight:800,color:m.color}}>{m.label}</div></div>:null;
            })()}
            <button onClick={e=>{e.stopPropagation();shareGamePick();}} style={{background:"rgba(139,92,246,.15)",border:"1px solid rgba(139,92,246,.35)",borderRadius:8,color:"#C4B5FD",fontSize:10,fontWeight:800,padding:"5px 10px",cursor:"pointer"}}>📤 Share</button>
          </div>
        </div>
      )}
    </div>
      ...
      .then(data=>{
        const evts = (data.events||[]).map(evt=>{
          ...
          return {
            ...
            home:{name:home?.team?.shortDisplayName||home?.team?.displayName||"",score:home?.score!=null&&home?.score!="" ? home.score : (isLive||st?.completed?"0":""),logo:home?.team?.logo||"",color:home?.team?.color||"333",winner:home?.winner},
            away:{name:away?.team?.shortDisplayName||away?.team?.displayName||"",score:away?.score!=null&&away?.score!="" ? away.score : (isLive||st?.completed?"0":""),logo:away?.team?.logo||"",color:away?.team?.color||"333",winner:away?.winner},
            broadcast:comp?.broadcasts?.[0]?.names?.join(", ")||"",
            venue:comp?.venue?.fullName||"",
            city:comp?.venue?.address?.city||"",
...
// ─── ALL GAMES SECTION (Yesterday / Today / Tomorrow) ────────────────────────
const ALL_GAMES_LEAGUES = [
  {label:"⚽ World Cup",  path:"soccer/fifa.world"},
  {label:"⚽ EPL",        path:"soccer/eng.1"},
...
function AllGamesSection({ onBack }) {
  ...
  useEffect(() => {
    ...
    const dates = {yesterday:getDate(-1), today:getDate(0), tomorrow:getDate(1)};

    const fetchDay = async (dateStr) => {
      const results = await Promise.all(
        ALL_GAMES_LEAGUES.map(async l => {
          try {
            const r = await fetch(`https://site.api.espn.com/apis/site/v2/sports/${l.path}/scoreboard?dates=${dateStr}`);
            if (!r.ok) return [];
            ...
            return (d.events||[]).map(evt => {
              ...
              return {
                id: evt.id,
                league: l.label,
                home: {name:home?.team?.displayName||"", abbr:home?.team?.abbreviation||"", score:home?.score, logo:home?.team?.logo},
                away: {name:away?.team?.displayName||"", abbr:away?.team?.abbreviation||"", score:away?.score, logo:away?.team?.logo},
                time: evt.date,
      ...
      return results.flat().filter(g=>g.home.name);
    };
    ...
    Promise.all([
      ...
      setGames({yesterday,today,tomorrow});
      setLoading(false);
    }).catch(()=>setLoading(false));
  }, []);

  const dayGames = games[activeDay] || [];
  const DAY_LABELS = {
    yesterday:{label:"Yesterday", color:"var(--muted)"},
    today:    {label:"Today",     color:"#10B981"},
    tomorrow: {label:"Tomorrow",  color:"#8B5CF6"},
  };

  // Group by league
  const byLeague = dayGames.reduce((acc, g) => {
  ...
  return (
    <div style={{padding:"0 14px 100px"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
        <button onClick={onBack} style={{background:"rgba(255,255,255,.06)",border:"1px solid var(--border)",borderRadius:10,color:"var(--muted)",padding:"6px 12px",fontSize:12,cursor:"pointer",fontWeight:700}}>
          ← Back
        </button>
        <div>
          <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:18}}>🏟️ All Games</div>
          <div style={{fontSize:11,color:"var(--muted)"}}>All sports · All leagues</div>
        </div>
      </div>

      {/* Day tabs */}
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        {Object.entries(DAY_LABELS).map(([key,{label,color}])=>(
          <button key={key} onClick={()=>setActiveDay(key)}
            style={{
              flex:1,background:activeDay===key?`${color}18`:"rgba(255,255,255,.04)",
              border:`1.5px solid ${activeDay===key?color:"rgba(255,255,255,.08)"}`,
              borderRadius:12,padding:"10px 0",
              fontFamily:"var(--font-head)",fontWeight:800,fontSize:13,
              color:activeDay===key?color:"var(--muted)",cursor:"pointer",
              transition:"all .2s",
            }}>
            {label}
            {!loading&&<div style={{fontSize:10,fontWeight:600,opacity:.7,marginTop:2}}>{(games[key]||[]).length} games</div>}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {Array.from({length:6}).map((_,i)=><div key={i} className="skeleton" style={{height:76,borderRadius:14}}/>)}
        </div>
      ) : dayGames.length === 0 ? (
        <div style={{textAlign:"center",padding:"40px 0",color:"var(--muted)"}}>
          <div style={{fontSize:40,marginBottom:12}}>🏁</div>
          <div style={{fontWeight:700,fontSize:15}}>No games {activeDay}</div>
          <div style={{fontSize:12,marginTop:4}}>Check back for upcoming fixtures</div>
        </div>
      ) : (
        Object.entries(byLeague).map(([league, lgGames]) => (
          <div key={league} style={{marginBottom:20}}>
            <div style={{fontSize:11,fontWeight:800,color:"var(--muted)",letterSpacing:1,marginBottom:8,paddingLeft:2}}>{league}</div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {lgGames.map(g => (
                <div key={g.id} style={{
                  background:g.isLive?"rgba(239,68,68,.07)":"rgba(255,255,255,.03)",
                  border:`1px solid ${g.isLive?"rgba(239,68,68,.3)":g.isOver?"rgba(255,255,255,.06)":"rgba(139,92,246,.15)"}`,
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                      {g.away.logo&&<img src={g.away.logo} alt="" style={{width:20,height:20,objectFit:"contain"}} onError={e=>e.target.style.display="none"}/>}
                      <span style={{fontSize:13,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.away.abbr||g.away.name}</span>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      {g.home.logo&&<img src={g.home.logo} alt="" style={{width:20,height:20,objectFit:"contain"}} onError={e=>e.target.style.display="none"}/>}
                      <span style={{fontSize:13,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.home.abbr||g.home.name}</span>
                    </div>
                  </div>

                  {/* Score / Time */}
                  <div style={{textAlign:"center",flexShrink:0,minWidth:60}}>
                    {(g.isLive||g.isOver) && g.away.score!==undefined ? (
                      <>
                        <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:18,lineHeight:1.2}}>
                          <span style={{color:g.isLive?"#ef4444":"#fff"}}>{g.away.score}</span>
                          <span style={{color:"var(--muted)",margin:"0 4px",fontSize:14}}>-</span>
                          <span style={{color:g.isLive?"#ef4444":"#fff"}}>{g.home.score}</span>
                        </div>
                        {g.isLive&&<div style={{fontSize:9,color:"#ef4444",fontWeight:800,letterSpacing:.5,marginTop:2}}>● LIVE</div>}
                        {g.isOver&&<div style={{fontSize:9,color:"var(--muted)",marginTop:2}}>Final</div>}
                      </>
                    ) : (
                      <div style={{fontSize:13,fontWeight:700,color:"#8B5CF6"}}>{fmtTime(g.time)}</div>
                    )}
                  </div>
                </div>
              ))}
...
const SPORT_CARD_COLOR = "#a3c4f3"; // uniform blue-white for all regular sport tabs
const SPORT_CARDS = [
  { emoji:"🏀", name:"NBA",        query:"NBA basketball",         color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:2,   service:"ESPN+/Max",  glow:"rgba(163,196,243,.25)" },
  { emoji:"🏈", name:"NFL",        query:"NFL football",           color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:2.5, service:"ESPN/Fox",   glow:"rgba(163,196,243,.25)" },
  { emoji:"⚽", name:"Soccer",     query:"soccer_hub",             color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:2,   service:"All Leagues",glow:"rgba(163,196,243,.25)" },
  { emoji:"⚾", name:"MLB",        query:"MLB baseball",           color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:2,   service:"Apple TV+",  glow:"rgba(163,196,243,.25)" },
  { emoji:"🥊", name:"Boxing/UFC", query:"UFC mixed martial arts", color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:1.5, service:"ESPN+",      glow:"rgba(163,196,243,.25)" },
  { emoji:"🏒", name:"NHL",        query:"NHL hockey",             color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:2,   service:"ESPN+",      glow:"rgba(163,196,243,.25)" },
  { emoji:"🏈", name:"College",    query:"college football NCAA",  color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:2,   service:"ESPN/Fox",   glow:"rgba(163,196,243,.25)" },
  { emoji:"💪", name:"WWE",        query:"WWE wrestling",          color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:2,   service:"Peacock",    glow:"rgba(163,196,243,.25)" },
  { emoji:"🏎️",name:"Formula 1",  query:"Formula 1 racing",       color:SPORT_CARD_COLOR, font:"'Bebas Neue',var(--font-head)", letterSpacing:1.5, service:"ESPN+",      glow:"rgba(163,196,243,.25)" },
  { emoji:"🏅", name:"Olympics",   query:"Olympics sports",        color:"#FFD700",        font:"'Bebas Neue',var(--font-head)", letterSpacing:2,   service:"Peacock",    glow:"rgba(255,215,0,.35)",  special:true },
];
...
// ── Shared team search logic ──────────────────────────────────────────────────
async function fetchTeamGame(nameOrQuery) {
  const term = (nameOrQuery||"").trim().toLowerCase();
  if (!term) return null;
  const results = await Promise.all(
  ...
  return live||upcoming||finished||null;
}

// Reusable result card
function TeamGameResult({ result, loading, searched, query }) {
  if (loading) return (
    <div style={{padding:"12px 14px",display:"flex",gap:8}}>
      {[1,2,3].map(i=><div key={i} className="skeleton" style={{flex:1,height:48,borderRadius:8}}/>)}
    </div>
  );
  if (searched && !result) return (
    <div style={{padding:"12px 14px",textAlign:"center",color:"var(--muted)",fontSize:12}}>
      No games found for "<strong>{query}</strong>" — try a different spelling.
    </div>
  );
  if (!result) return null;
  return (
    <div style={{padding:"10px 14px"}}>
      <div style={{fontSize:9,color:"var(--muted)",fontWeight:700,letterSpacing:1,marginBottom:6}}>{result.sport.toUpperCase()}</div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{flex:1,minWidth:0}}>
          {[result.away,result.home].map((t,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:6,marginBottom:i===0?4:0}}>
              {t.logo&&<img src={t.logo} alt="" style={{width:20,height:20,objectFit:"contain",flexShrink:0}}/>}
              <span style={{fontWeight:700,fontSize:13,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.name}</span>
              {(result.isLive||result.isOver)&&<span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:14,color:t.winner?"#fbbf24":"#f1f5f9",flexShrink:0}}>{t.score}</span>}
            </div>
          ))}
        </div>
        <div style={{flexShrink:0,textAlign:"center",minWidth:90}}>
          {result.isLive
            ? <div style={{background:"rgba(239,68,68,.15)",border:"1px solid rgba(239,68,68,.4)",borderRadius:8,padding:"5px 8px",color:"#ef4444",fontWeight:800,fontSize:10}}>🔴 LIVE<br/><span style={{fontSize:9}}>{result.period}</span></div>
            : result.isOver
              ? <div style={{fontSize:10,color:"var(--muted)",fontWeight:700}}>FINAL</div>
              : <div style={{background:"rgba(139,92,246,.1)",border:"1px solid rgba(139,92,246,.3)",borderRadius:8,padding:"5px 8px"}}>
                  <div style={{fontSize:10,fontWeight:700,color:"#C4B5FD"}}>{result.localDate}</div>
                  <div style={{fontSize:9,color:"var(--muted)",marginTop:1}}>{result.localTime}</div>
                  {result.broadcast&&<div style={{fontSize:8,color:"var(--gold)",marginTop:2,fontWeight:700}}>{result.broadcast}</div>}
                </div>
          }
        </div>
      </div>
      {result.venue&&<div style={{marginTop:6,fontSize:9,color:"var(--muted)"}}>📍 {result.venue}{result.city?`, ${result.city}`:""}</div>}
    </div>
  );
}

// ── Predictions mini-pill for embedding inside My Teams ──────────────────────
function PredictionsPill({ onClick }) {
  const s = getPredStats();
  const acc = s.total > 0 ? Math.round(s.correct/s.total*100) : 0;
  const milestone = [...PRED_MILESTONES].reverse().find(m=>s.streak>=m.n);
  ...
  const sharePredictions = () => {
    const text = s.total > 0
      ? `🔮 I'm on a ${s.streak}-game prediction streak on The StreamHub!\n\n⚡ ${s.streak} correct in a row · 🏆 Best: ${s.best} · 📊 ${s.correct}/${s.total} (${acc}% accuracy)${milestone?`\n\n${milestone.icon} ${milestone.label} badge unlocked!`:""}\n\nThink you can beat my streak? Try predictions at thestreamhub.app 👀`
      : `🔮 I just started predicting sports games on The StreamHub!\n\nPick who wins every game. Can you get a longer streak than me?\n\nTry it free → thestreamhub.app`;
    navigator.share
      ? navigator.share({title:"My StreamHub Predictions", text, url:"https://thestreamhub.app"}).catch(()=>{})
      : navigator.clipboard?.writeText(text).then(()=>{});
  };
    ...
    <div style={{
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          ...
          <span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:12,color:"#111"}}>
            Predictions {milestone&&<span style={{fontSize:12}}>{milestone.icon}</span>}
          </span>
        </div>
        <button onClick={sharePredictions} style={{
          background:"rgba(0,0,0,.06)",border:"1px solid rgba(0,0,0,.12)",
          borderRadius:99,padding:"2px 8px",fontSize:9,fontWeight:800,
      ...
      {/* Stats row */}
      {s.total === 0 ? (
        <div style={{fontSize:9,color:"rgba(0,0,0,.4)",textAlign:"center"}}>Tap any game to make your first prediction</div>
      ) : (
        <div style={{display:"flex",gap:5}}>
          {[
            ...
            <div key={st.label} style={{
              flex:1, background:st.bg, border:`1.5px solid ${st.border}`,
              borderRadius:7, padding:"4px 3px", textAlign:"center",
              minWidth:0, overflow:"hidden",
            }}>
              <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:12,color:st.color,lineHeight:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{st.val}</div>
              <div style={{fontSize:7,color:st.color,opacity:.7,fontWeight:700,marginTop:2}}>{st.label}</div>
            </div>
          ))}
        </div>
      )}
...
// ── My Teams + Find Any Team — combined banner ────────────────────────────────
function FavoriteTeamsBanner({ favoriteTeams, compact=false }) {
  const allFavTeams = Object.entries(favoriteTeams||{}).flatMap(([sport,v])=>{
    const arr = Array.isArray(v)?v:(v?[v]:[]);
    return arr.map(name=>({sport,name}));
  });

  // Restore last selected team from localStorage, fallback to first team
  const savedTeam = localStorage.getItem("sh_last_team");
  const defaultTeam = allFavTeams.find(t=>t.name===savedTeam)?.name || allFavTeams[0]?.name || null;
  ...
  useEffect(() => {
    // On mount: load the saved team or default to first
    const saved = localStorage.getItem("sh_last_team");
    const target = allFavTeams.find(t=>t.name===saved)?.name || allFavTeams[0]?.name;
    if (target) { setActiveTeam(target); loadTeam(target); }
  }, [JSON.stringify(favoriteTeams)]);

  const loadTeam = async (name) => {
    setActiveTeam(name);
    localStorage.setItem("sh_last_team", name); // persist selection
    setDropOpen(false);
    setShowSearchResult(false);
    setLoading(true);
    setResult(null);
    const r = await fetchTeamGame(name).catch(()=>null);
    setResult(r);
    setLoading(false);
  };
  ...
  const searchTeam = async () => {
    ...
    const r = await fetchTeamGame(term).catch(()=>null);
    setSearchResult(r);
    setSearching(false);
  };
  ...
  const GameCard = ({r, l}) => {
    if (l) return <div className="skeleton" style={{height:32,borderRadius:8,marginTop:6}}/>;
    if (!r) return null;
    return (
      <div style={{background:"linear-gradient(135deg,#1e3a5f,#0f2744)",border:"1.5px solid rgba(96,165,250,.4)",borderRadius:12,padding:"8px 10px",marginTop:6}}>
        {/* Sport label + status */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
          <div style={{fontSize:9,color:"#93c5fd",fontWeight:800,letterSpacing:1}}>{r.sport.toUpperCase()}</div>
          {r.isLive
            ? <div style={{background:"#ef4444",borderRadius:99,padding:"1px 7px",color:"#fff",fontWeight:900,fontSize:8,animation:"pulse 1.5s infinite"}}>🔴 LIVE</div>
            : r.isOver
              ? <div style={{fontSize:8,color:"rgba(255,255,255,.45)",fontWeight:700,letterSpacing:.5}}>FINAL</div>
              : <div style={{textAlign:"right"}}>
                  <div style={{fontSize:9,fontWeight:800,color:"#fbbf24"}}>{r.localDate}</div>
                  <div style={{fontSize:8,color:"rgba(255,255,255,.5)"}}>{r.localTime}</div>
                </div>
          }
        </div>
        {/* Teams — each with white logo bg pill */}
        {[r.away,r.home].map((t,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:i===0?6:0}}>
            {/* Logo on white circle so colours always show */}
            <div style={{width:28,height:28,borderRadius:"50%",background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 2px 8px rgba(0,0,0,.4)"}}>
              {t.logo
                ? <img src={t.logo} alt="" style={{width:20,height:20,objectFit:"contain"}}/>
                : <div style={{width:24,height:24,borderRadius:"50%",background:`#${t.color||"3b82f6"}`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:8,fontWeight:900}}>{(t.name||"?").slice(0,2).toUpperCase()}</div>
              }
            </div>
            <span style={{fontSize:12,fontWeight:700,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"#f1f5f9"}}>{t.name}</span>
            {(r.isLive||r.isOver)&&<span style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:15,color:t.winner?"#fbbf24":"#e2e8f0",flexShrink:0,minWidth:20,textAlign:"right"}}>{t.score}</span>}
          </div>
        ))}
      </div>
    );
  ...
  return (
    <div style={{
      ...
      {/* Title */}
      <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:compact?11:13,color:"#60a5fa",marginBottom:6}}><span style={{fontSize:compact?18:24,marginRight:4}}>🛡️</span>My Teams</div>

      {/* Single team pill — acts as dropdown trigger */}
      {allFavTeams.length===0
        ? <div style={{fontSize:9,color:"rgba(96,165,250,.6)",marginBottom:6}}>Tap a sport to follow teams</div>
        : <div style={{position:"relative",marginBottom:6}}>
            <button onClick={()=>setDropOpen(o=>!o)}
              style={{
                ...
                color:"var(--gold)",fontSize:fs,fontWeight:700,cursor:"pointer",
              }}>
              <span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1,textAlign:"left"}}>
                {activeTeam||"Select team"}
              </span>
              <span style={{marginLeft:6,fontSize:9,opacity:.7,flexShrink:0}}>{dropOpen?"▲":"▼"}</span>
            </button>
            {/* Dropdown */}
            {dropOpen && (
              <div style={{
                ...
                scrollbarWidth:"thin", scrollbarColor:"rgba(59,130,246,.3) transparent",
              }}>
                {allFavTeams.map(({sport,name})=>(
                  <button key={sport+name} onClick={()=>loadTeam(name)}
                    style={{
                      display:"block",width:"100%",textAlign:"left",
                      background:activeTeam===name?"rgba(59,130,246,.15)":"transparent",
                      border:"none",borderBottom:"1px solid rgba(255,255,255,.05)",
                      color:activeTeam===name?"#60a5fa":"var(--text)",
                      padding:"10px 14px",fontSize:13,fontWeight:activeTeam===name?700:400,
                      cursor:"pointer",
                    }}
                    onMouseEnter={e=>e.currentTarget.style.background="rgba(59,130,246,.1)"}
                    onMouseLeave={e=>e.currentTarget.style.background=activeTeam===name?"rgba(59,130,246,.15)":"transparent"}>
                    {name} <span style={{fontSize:10,color:"var(--muted)",fontWeight:400}}>{sport}</span>
                  </button>
                ))}
              </div>
            )}
      ...
      {/* Game result for active team */}
      {!showSearchResult && <GameCard r={result} l={loading}/>}

      {/* Predictions pill — embedded white card */}
      <PredictionsPill/>
      ...
      {activeTeam && (
        <button onClick={()=>{
          const text = result
            ? result.isLive
              ? `🏆 ${result.away?.name} vs ${result.home?.name} is LIVE right now!\n\nTracking my ${activeTeam} on The StreamHub — live scores + where to watch every game.\n\nCheck it out → thestreamhub.app`
              : result.isOver
                ? `🏆 Just caught the ${result.away?.name} vs ${result.home?.name} final score on The StreamHub!\n\nTracking my ${activeTeam} all season — live scores, schedules & where to watch.\n\nJoin me → thestreamhub.app`
                : `🏆 ${activeTeam} plays ${result.away?.name === activeTeam ? result.home?.name : result.away?.name} on ${result.localDate}!\n\nTracking all my teams on The StreamHub — live scores + where to watch every game.\n\nJoin me → thestreamhub.app`
            : `🏆 I'm tracking my ${activeTeam} on The StreamHub!\n\nLive scores, schedules & where to watch every game — all in one place.\n\nCheck it out → thestreamhub.app`;
          navigator.share
            ? navigator.share({title:`My ${activeTeam} on StreamHub`, text, url:"https://thestreamhub.app"}).catch(()=>{})
            : navigator.clipboard?.writeText(text).then(()=>{});
        }}
        style={{
          width:"100%", marginTop:compact?6:8,
          background:"rgba(59,130,246,.1)", border:"1.5px solid rgba(59,130,246,.3)",
          borderRadius:8, color:"#60a5fa", padding:"6px 0",
          fontSize:11, fontWeight:800, cursor:"pointer",
          display:"flex", alignItems:"center", justifyContent:"center", gap:5,
        }}>📤 Share with a friend</button>
      )}
      ...
      {/* Search bar */}
      <div style={{marginTop:compact?5:7,paddingTop:compact?4:6,borderTop:"1px solid rgba(59,130,246,.1)"}}>
        <div style={{fontSize:9,color:"rgba(96,165,250,.55)",fontWeight:700,letterSpacing:.5,marginBottom:4}}>🔍 SEARCH ANY TEAM</div>
        <div style={{display:"flex",gap:5}}>
          <input
            value={query}
            onChange={e=>setQuery(e.target.value)}
            onKeyDown={e=>e.key==="Enter"&&searchTeam()}
            placeholder="Lakers, Arsenal, Cowboys…"
            style={{flex:1,background:"rgba(0,0,0,.2)",border:"1px solid rgba(59,130,246,.25)",borderRadius:7,color:"var(--text)",padding:compact?"4px 8px":"5px 8px",fontSize:10,outline:"none",minWidth:0}}
          />
          <button onClick={searchTeam}
            style={{background:"#3b82f6",border:"none",borderRadius:7,color:"#fff",padding:compact?"4px 9px":"5px 10px",fontWeight:800,fontSize:10,cursor:"pointer",flexShrink:0}}>
            {searching?"…":"Go"}
          </button>
        </div>
        {showSearchResult && <GameCard r={searchResult} l={searching}/>}
        {showSearchResult&&searched&&!searchResult&&!searching&&
          <div style={{fontSize:9,color:"var(--muted)",textAlign:"center",padding:"6px 0"}}>No games found for "{query}"</div>
        }
      </div>
    </div>
  );
...
// TeamNextGameSearch — kept as alias for backward compat but no longer rendered standalone
function TeamNextGameSearch({ favoriteTeams }) { return null; }
...
function SportCategoryGrid({ onSearch, favoriteTeams }) {
  return (
    <div style={{marginBottom:20}}>
      {/* MLB 2026 Banner */}
      <div onClick={()=>onSearch("MLB baseball")}
        style={{
          background:"linear-gradient(160deg,#0a0505 0%,#1a0808 45%,#0d0303 100%)",
          border:"2px solid rgba(200,16,46,.5)",
          borderRadius:16, marginBottom:12,
          cursor:"pointer", position:"relative", overflow:"hidden",
          boxShadow:"0 8px 32px rgba(200,16,46,.2), 0 0 60px rgba(200,16,46,.06)",
          transition:"all .2s", minHeight:160,
        }}
        onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(200,16,46,.9)";e.currentTarget.style.transform="scale(1.005)";}}
        onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(200,16,46,.5)";e.currentTarget.style.transform="scale(1)";}}>

        {/* Baseball diamond SVG — left side */}
        <div style={{position:"absolute",left:8,top:0,bottom:0,width:110,pointerEvents:"none",opacity:.2,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <svg viewBox="0 0 100 100" width="100" height="100">
            <g stroke="#C8102E" strokeWidth="4" strokeLinecap="round" fill="none">
              {/* Diamond */}
              <polygon points="50,5 95,50 50,95 5,50" stroke="#C8102E" strokeWidth="3"/>
              {/* Bases */}
              <rect x="44" y="0" width="12" height="10" rx="2" fill="#fff" stroke="none"/>
              <rect x="90" y="44" width="10" height="12" rx="2" fill="#fff" stroke="none"/>
              <rect x="44" y="90" width="12" height="10" rx="2" fill="#fff" stroke="none"/>
              <rect x="0" y="44" width="10" height="12" rx="2" fill="#fff" stroke="none"/>
              {/* Pitcher mound */}
              <circle cx="50" cy="50" r="5" fill="#C8102E" stroke="none"/>
            </g>
          </svg>
        </div>

        {/* Animated baseballs — right side */}
        {[
          {s:34, t:"6%",   l:"55%", d:"8s",  dl:"0s",   o:.38},
          {s:22, t:"58%",  l:"65%", d:"10s", dl:"1.4s", o:.3},
          {s:40, t:"4%",   l:"72%", d:"12s", dl:"2.8s", o:.25},
          {s:18, t:"65%",  l:"80%", d:"9s",  dl:"0.8s", o:.35},
          {s:28, t:"18%",  l:"85%", d:"11s", dl:"3.2s", o:.28},
          {s:16, t:"72%",  l:"91%", d:"13s", dl:"1.6s", o:.25},
          {s:46, t:"-8%",  l:"62%", d:"15s", dl:"0.4s", o:.16},
          {s:20, t:"48%",  l:"95%", d:"10s", dl:"4.2s", o:.28},
        ].map((b,i)=>(
          <div key={i} style={{
            position:"absolute", top:b.t, left:b.l,
            fontSize:b.s, opacity:b.o, pointerEvents:"none",
            animation:`wcFloat ${b.d} ease-in-out infinite`,
            animationDelay:b.dl, userSelect:"none",
          }}>⚾</div>
        ))}

        {/* Scrolling team strip */}
        {(()=>{
          const TEAMS = [
            {code:"NYY",color:"#003087"},{code:"LAD",color:"#005A9C"},
            {code:"ATL",color:"#CE1141"},{code:"HOU",color:"#EB6E1F"},
            {code:"PHI",color:"#E81828"},{code:"SD",color:"#2F241D"},
            {code:"BOS",color:"#BD3039"},{code:"CHC",color:"#0E3386"},
            {code:"SF",color:"#FD5A1E"}, {code:"TB",color:"#092C5C"},
            {code:"MIL",color:"#12284B"},{code:"CLE",color:"#E31937"},
            {code:"MIN",color:"#002B5C"},{code:"SEA",color:"#005C5C"},
            {code:"TOR",color:"#134A8E"},{code:"BAL",color:"#DF4601"},
            {code:"NYM",color:"#002D72"},{code:"STL",color:"#C41E3A"},
            {code:"CIN",color:"#C6011F"},{code:"DET",color:"#0C2340"},
            {code:"KC",color:"#004687"}, {code:"OAK",color:"#003831"},
            {code:"WSH",color:"#AB0003"},{code:"MIA",color:"#00A3E0"},
            {code:"COL",color:"#33006F"},{code:"ARI",color:"#A71930"},
            {code:"LAA",color:"#BA0021"},{code:"TEX",color:"#003278"},
            {code:"PIT",color:"#FDB827"},{code:"CWS",color:"#27251F"},
          ];
          const Strip = ({sfx}) => TEAMS.map((t,i)=>(
            <span key={sfx+i} style={{
              display:"inline-flex",alignItems:"center",justifyContent:"center",
              width:32,height:17,borderRadius:3,background:t.color,
              color:t.color==="#FDB827"?"#000":"#fff",
              fontSize:7.5,fontWeight:900,letterSpacing:.3,marginRight:5,flexShrink:0,
            }}>{t.code}</span>
          ));
          return (
            <div style={{position:"absolute",bottom:0,left:0,right:0,height:26,overflow:"hidden",pointerEvents:"none",background:"rgba(0,0,0,.35)"}}>
              <div style={{display:"inline-flex",alignItems:"center",padding:"4px 0",whiteSpace:"nowrap",animation:"wcFlags 32s linear infinite"}}>
                <Strip sfx="a"/><Strip sfx="b"/>
              </div>
            </div>
          );
        })()}
        ...
        {/* Main content */}
        <div style={{position:"relative",zIndex:1,padding:"18px 20px 34px",display:"flex",flexDirection:"column",alignItems:"center",gap:10,textAlign:"center"}}>

          <div style={{
            fontFamily:"'Bebas Neue', var(--font-head)", fontWeight:400,
            fontSize:"clamp(26px,4.5vw,46px)", letterSpacing:3, lineHeight:1, color:"#fff",
            textShadow:"0 0 30px rgba(200,16,46,.7), 0 0 60px rgba(200,16,46,.35), 0 2px 4px rgba(0,0,0,.9)",
          }}>⚾ MLB 2026 Season</div>

          <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            <div style={{background:"#C8102E",borderRadius:99,padding:"3px 12px",fontSize:9,fontWeight:900,color:"#fff",letterSpacing:.8,boxShadow:"0 0 12px rgba(200,16,46,.7)",animation:"pulse 1.5s infinite"}}>🔴 IN SEASON</div>
            <div style={{background:"rgba(255,255,255,.12)",borderRadius:99,padding:"3px 10px",fontSize:9,fontWeight:900,color:"#fff",letterSpacing:.8,border:"1px solid rgba(255,255,255,.3)"}}>30 TEAMS</div>
            <div style={{background:"rgba(200,16,46,.2)",borderRadius:99,padding:"3px 10px",fontSize:9,fontWeight:900,color:"#ff6b8a",letterSpacing:.8,border:"1px solid rgba(200,16,46,.4)"}}>ENDS OCT 4</div>
          </div>

          <div style={{display:"flex",justifyContent:"center",gap:8,flexWrap:"wrap"}}>
            {[
              {icon:"📺", text:"ESPN · Fox · Apple TV+ · Peacock"},
              {icon:"🏆", text:"World Series — Oct 22, 2026"},
              {icon:"⚾", text:"162 games per team · 2,430 total"},
            ].map((h,i)=>(
              <div key={i} style={{
                display:"flex",alignItems:"center",gap:6,
                background:"rgba(255,255,255,.07)",borderRadius:99,
                padding:"5px 12px",border:"1px solid rgba(200,16,46,.25)",
              }}>
                <span style={{fontSize:11}}>{h.icon}</span>
                <span style={{fontSize:10,fontWeight:700,color:"rgba(255,255,255,.9)",letterSpacing:.2}}>{h.text}</span>
              </div>
            ))}
          </div>
      ...
      <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontWeight:400,fontSize:18,color:"var(--sports)",marginBottom:12,letterSpacing:2}}>
        🏆 SELECT A SPORT
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>
        {SPORT_CARDS.map(s=>{
          const favV = favoriteTeams?.[s.name];
          const favTeams = Array.isArray(favV)?favV:(favV?[favV]:[]);
          const hasFav = favTeams.length > 0;
          return (
          <button key={s.name} onClick={()=>onSearch(s.query)}
            style={{
              background:s.special?"rgba(255,215,0,.05)":"rgba(255,255,255,.03)",
              border:`1.5px solid ${s.special?"rgba(255,215,0,.4)":hasFav ? s.color+"99" : "rgba(255,255,255,.08)"}`,
              borderRadius:14, padding:"14px 14px 12px",
              display:"flex",flexDirection:"column",gap:8,
              cursor:"pointer", textAlign:"left",
              transition:"all .2s",
              boxShadow:hasFav?`0 0 16px ${s.glow}`:"none",
              position:"relative", overflow:"hidden",
            }}
            onMouseEnter={e=>{
            ...
            onMouseLeave={e=>{
              e.currentTarget.style.borderColor=hasFav?`${s.color}99`:"rgba(255,255,255,.08)";
              e.currentTarget.style.transform="scale(1)";
              e.currentTarget.style.boxShadow=hasFav?`0 0 16px ${s.glow}`:"none";
              e.currentTarget.style.background="rgba(255,255,255,.03)";
            }}>
            {/* Glow orb behind emoji */}
            <div style={{position:"absolute",top:-20,right:-20,width:80,height:80,borderRadius:"50%",background:s.glow,filter:"blur(25px)",opacity:.5,pointerEvents:"none"}}/>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",position:"relative"}}>
              <div>
                <div style={{fontSize:36,lineHeight:1,marginBottom:6}}>{s.emoji}</div>
                <div style={{fontFamily:s.font,fontWeight:400,fontSize:20,color:s.color,letterSpacing:s.letterSpacing,lineHeight:1,textShadow:`0 0 12px ${s.glow}`}}>{s.name}</div>
                <div style={{fontSize:10,color:"rgba(255,255,255,.4)",marginTop:3,fontWeight:600}}>{s.service}</div>
              </div>
              {hasFav && <div style={{background:s.color,borderRadius:99,padding:"2px 7px",fontSize:8,fontWeight:900,color:"#000",letterSpacing:.5,flexShrink:0,alignSelf:"flex-start"}}>★ FAV</div>}
            </div>
            ...
            {favTeams.length>0&&(
              <div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:2}}>
                {favTeams.map(name=>{
                  ...
                  return (
                    <div key={name} style={{display:"flex",alignItems:"center",gap:4,background:`${s.color}22`,border:`1px solid ${s.color}55`,borderRadius:99,padding:"2px 6px 2px 4px"}}>
                      {logo
                      ...
                      <span style={{fontSize:9,fontWeight:700,color:s.color,whiteSpace:"nowrap",maxWidth:60,overflow:"hidden",textOverflow:"ellipsis"}}>{name}</span>
                    </div>
                  );
                })}
              </div>
...
// ─── STREAK TRACKER ──────────────────────────────────────────────────────────
function getStreak(savedStreak=null) {
  try {
    const today = new Date().toDateString();
    const yesterday = new Date(Date.now()-86400000).toDateString();
    // If Supabase has streak data, use it as source of truth
    if (savedStreak) {
      const data = typeof savedStreak==="string" ? JSON.parse(savedStreak) : savedStreak;
      if (data.lastVisit===today) {
        localStorage.setItem("streamhub_streak", JSON.stringify(data));
        return data.streak||1;
      }
      const streak = data.lastVisit===yesterday ? (data.streak||1)+1 : 1;
      const updated = {lastVisit:today,streak};
      localStorage.setItem("streamhub_streak", JSON.stringify(updated));
      return streak;
    }
...
// ─── REFERRAL MODAL ───────────────────────────────────────────────────────────
function ReferralModal({ onClose, user }) {
  const [copied, setCopied] = useState(false);
  const referralUrl = `https://thestreamhub.app${user ? `?ref=${user.id.slice(0,8)}` : ""}`;
  const shareText = "I've been using StreamHub to track all my streaming services + get AI picks for what to watch. It's free — check it out:";
  ...
  const shareViaText = () => {
    const sms = `sms:?&body=${encodeURIComponent(shareText + " " + referralUrl)}`;
    window.location.href = sms;
  };

  const shareViaTwitter = () => {
    const tw = `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(referralUrl)}`;
    window.open(tw, "_blank");
  };

  const shareNative = () => {
    if (navigator.share) {
      navigator.share({ title:"StreamHub", text:shareText, url:referralUrl }).catch(()=>{});
    } else { copyLink(); }
  };

  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:1100,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(10px)",animation:"fadeIn .2s"}}>
      <div onClick={e=>e.stopPropagation()} className="fadeUp" style={{background:"var(--surface)",borderRadius:22,width:"100%",maxWidth:420,border:"1px solid var(--border)",boxShadow:"0 40px 80px rgba(0,0,0,.85)",overflow:"hidden"}}>

        {/* Header */}
        <div style={{background:"linear-gradient(135deg,rgba(139,92,246,.15),rgba(245,158,11,.1))",padding:"24px 24px 20px",borderBottom:"1px solid var(--border)",position:"relative"}}>
          <button onClick={onClose} style={{position:"absolute",top:16,right:16,background:"none",border:"none",color:"var(--muted)",fontSize:18,cursor:"pointer"}}>✕</button>
          <div style={{fontSize:36,marginBottom:8}}>🎉</div>
          <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:20,marginBottom:4}}>Share StreamHub</div>
          <div style={{fontSize:13,color:"var(--muted)",lineHeight:1.5}}>Help your friends find what to watch — send them your link</div>
        </div>

        {/* Link copy */}
        <div style={{padding:"20px 24px 0"}}>
          <div style={{fontSize:11,fontWeight:700,color:"var(--muted)",letterSpacing:1,marginBottom:8,fontFamily:"var(--font-head)"}}>YOUR SHARE LINK</div>
          <div style={{display:"flex",gap:8,alignItems:"center",background:"rgba(255,255,255,.05)",border:"1px solid var(--border)",borderRadius:11,padding:"10px 14px"}}>
            <div style={{flex:1,fontSize:12,color:"var(--muted)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{referralUrl}</div>
            <button onClick={copyLink}
              style={{flexShrink:0,background:copied?"var(--sports)":"rgba(245,158,11,.15)",border:`1px solid ${copied?"var(--sports)":"rgba(245,158,11,.3)"}`,borderRadius:8,color:copied?"#fff":"var(--gold)",padding:"5px 12px",fontSize:11,fontWeight:700,cursor:"pointer",transition:"all .2s",whiteSpace:"nowrap"}}>
              {copied ? "✓ Copied!" : "Copy"}
            </button>
          </div>
        </div>
        ...
        <div style={{padding:"16px 24px 24px",display:"flex",flexDirection:"column",gap:10}}>
          <div style={{fontSize:11,fontWeight:700,color:"var(--muted)",letterSpacing:1,marginBottom:2,fontFamily:"var(--font-head)"}}>SHARE VIA</div>

          <button onClick={shareViaText}
            style={{display:"flex",alignItems:"center",gap:14,background:"rgba(16,185,129,.08)",border:"1.5px solid rgba(16,185,129,.25)",borderRadius:13,padding:"13px 16px",cursor:"pointer",transition:"all .2s",textAlign:"left"}}
            ...
            <div>
              <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,color:"#10B981"}}>iMessage / SMS</div>
              <div style={{fontSize:11,color:"var(--muted)"}}>Open in Messages app</div>
            </div>
          </button>

          <button onClick={shareViaTwitter}
            style={{display:"flex",alignItems:"center",gap:14,background:"rgba(29,161,242,.08)",border:"1.5px solid rgba(29,161,242,.25)",borderRadius:13,padding:"13px 16px",cursor:"pointer",transition:"all .2s",textAlign:"left"}}
            onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(29,161,242,.6)"}
            onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(29,161,242,.25)"}>
            <span style={{fontSize:22,fontWeight:900,color:"#1DA1F2",fontFamily:"Georgia"}}>𝕏</span>
            <div>
              <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,color:"#1DA1F2"}}>Post on X / Twitter</div>
              <div style={{fontSize:11,color:"var(--muted)"}}>Share with your followers</div>
            </div>
          </button>

          {typeof navigator.share !== "undefined" && (
            <button onClick={shareNative}
              style={{display:"flex",alignItems:"center",gap:14,background:"rgba(139,92,246,.08)",border:"1.5px solid rgba(139,92,246,.25)",borderRadius:13,padding:"13px 16px",cursor:"pointer",transition:"all .2s",textAlign:"left"}}
              onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(251,146,60,.6)"}
              onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(139,92,246,.25)"}>
              <span style={{fontSize:24}}>📤</span>
              <div>
                <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,color:"#C4B5FD"}}>More Options</div>
                <div style={{fontSize:11,color:"var(--muted)"}}>AirDrop, Instagram, WhatsApp…</div>
              </div>
            </button>
          )}
        </div>
...
{"title":"Movie Name","year":2023,"type":"movie","service":"Netflix","reason":"One sentence why this is perfect for right now","duration":"1h 52m","vibe":"gripping thriller"}`;
    if (!checkRateLimit("watch_tonight")) {
      return;
    }
    try {
        ...
        <div style={{padding:20}}>
          ...
          {pick && !loading && (
            <div>
              ...
              {/* AI Reason */}
              <div style={{background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:12,padding:"12px 14px",marginBottom:16}}>
...
function PrivacyModal({ onClose }) {
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"var(--surface)",borderRadius:20,width:"100%",maxWidth:600,maxHeight:"85vh",border:"1px solid var(--border)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"18px 20px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:17}}>🔒 Privacy Policy</div>
          <button onClick={onClose} style={{background:"rgba(255,255,255,.08)",border:"none",borderRadius:8,color:"var(--muted)",width:30,height:30,fontSize:15,cursor:"pointer"}}>✕</button>
        </div>
        <div style={{overflowY:"auto",padding:"20px 24px",fontSize:13,lineHeight:1.8,color:"rgba(240,240,250,.75)"}}>
          <p style={{marginBottom:16,fontSize:11,color:"var(--muted)"}}>Last updated: June 27, 2026</p>
          <p style={{marginBottom:12,background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:10,padding:"10px 14px",color:"#C4B5FD",fontSize:12}}>The short version: We collect only what we need to run the service. We don't sell your data. We don't show you ads. You can delete your account and all your data at any time.</p>
          {[
            ["Who We Are","The StreamHub is an AI-powered streaming discovery platform at thestreamhub.app. For privacy questions: privacy@thestreamhub.app"],
            ["Information We Collect","• Email address and username when you create an account\n• Profile picture if you upload one\n• Streaming service preferences\n• Watchlist (movie/TV IDs you save)\n• Ratings and reviews you submit\n• Usage data and device info (browser type, screen size)\n• Sports predictions you make\n• Payment info is handled entirely by Stripe — we never see your card details"],
            ["How We Use Your Information","• To provide and operate the StreamHub service\n• To personalize your experience (AI picks, For You, Watch Tonight)\n• To sync your watchlist and ratings across devices\n• To process payments and manage your subscription\n• To send account-related emails (receipts, password resets)\n• To improve the app based on usage patterns"],
            ["Third-Party Services","• Supabase — authentication and database\n• Stripe — payment processing\n• Anthropic (Claude AI) — AI features (Mood Search, Watch Tonight, For You)\n• TMDB — movie and TV data\n• ESPN API — live sports scores\n• Vercel — hosting\n\nWe do not sell your data to any third party."],
            ["Your Rights","You have the right to access, correct, delete, or export your data at any time. EU and UK users have additional GDPR rights. Email privacy@thestreamhub.app to exercise any right."],
            ["Children's Privacy","The StreamHub is not directed at children under 13. We do not knowingly collect data from children under 13."],
            ["Changes","We may update this policy. Significant changes will be notified via email or in-app notice."],
          ].map(([title, body]) => (
            <div key={title} style={{marginBottom:18}}>
              <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,color:"var(--gold)",marginBottom:6}}>{title}</div>
...
function TermsModal({ onClose }) {
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(8px)"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"var(--surface)",borderRadius:20,width:"100%",maxWidth:600,maxHeight:"85vh",border:"1px solid var(--border)",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{padding:"18px 20px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:17}}>📋 Terms of Service</div>
          <button onClick={onClose} style={{background:"rgba(255,255,255,.08)",border:"none",borderRadius:8,color:"var(--muted)",width:30,height:30,fontSize:15,cursor:"pointer"}}>✕</button>
        </div>
        <div style={{overflowY:"auto",padding:"20px 24px",fontSize:13,lineHeight:1.8,color:"rgba(240,240,250,.75)"}}>
          <p style={{marginBottom:16,fontSize:11,color:"var(--muted)"}}>Last updated: June 27, 2026</p>
          <p style={{marginBottom:12,background:"rgba(245,158,11,.07)",border:"1px solid rgba(245,158,11,.2)",borderRadius:10,padding:"10px 14px",color:"#FCD34D",fontSize:12}}>By using The StreamHub, you agree to these terms. If you don't agree, please don't use the service.</p>
          {[
            ["Acceptance","These Terms govern your access to The StreamHub at thestreamhub.app. You must be at least 13 years old to use the service."],
            ["Free Tier — Always Free","• Mood Search (unlimited)\n• Sports Hub and live World Cup scores\n• Content browsing across all platforms\n• Watchlist and ratings\n• One free AI pick (Watch Tonight)"],
            ["Premium Tier — from $4.99/mo","• Unlimited Watch Tonight AI picks\n• For You personalized recommendations\n• Unlimited AI Cost Reports\n• Leaving Soon alerts\n• Full streaming stats\n\nPlans: Monthly $4.99/mo · Annual $24.99/yr · Lifetime $19.99 one-time"],
            ["Payments & Cancellation","Payments are processed by Stripe. You may cancel anytime — access continues until the end of your billing period. We offer a full refund within 7 days of your initial purchase. Email support@thestreamhub.app to request one."],
            ["Acceptable Use","You agree not to scrape, reverse-engineer, or abuse the service. You may not share accounts or circumvent subscription limits. We reserve the right to suspend accounts that violate these terms."],
            ["Disclaimers","The service is provided 'as is'. Sports scores and streaming availability are for informational purposes and may not always be current. We are not affiliated with Netflix, Hulu, Disney+, or any other streaming platform."],
            ["Governing Law","These Terms are governed by the laws of the United States."],
            ["Contact","support@thestreamhub.app"],
          ].map(([title, body]) => (
            <div key={title} style={{marginBottom:18}}>
              <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,color:"var(--gold)",marginBottom:6}}>{title}</div>
              <div style={{whiteSpace:"pre-line"}}>{body}</div>
...
function SubscriptionManagerPanel({ userSubs: initialSubs=[], onToggle, onDone }) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:0,height:"100%"}}>
      {/* Summary bar */}
      <div style={{background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:14,padding:"14px 16px",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
...
function ProfileModal({ user, profile, tier, watchlist, userRatings, userSubs=[], onClose, onSignOut, onUpgrade, showToast, onEditSubs, onSelectMovie, notifPermission, onRequestNotif, streak, onSaveSubs, onPrivacy, onTerms }) {
  ...
  // Share profile
  const shareProfile = () => {
    const text = `🎬 Check out my StreamHub profile!\n\nI use StreamHub — the AI streaming guide that finds what to watch across Netflix, Hulu, Disney+, Max, Apple TV+ and more. Live World Cup scores, mood search, and smarter streaming all in one place.\n\n✦ ${watchlist.length} titles saved · ${totalRatings} rated\n\nTry it free → thestreamhub.app`;
    if (navigator.share) {
      navigator.share({title:"My StreamHub Profile", text, url:"https://thestreamhub.app"}).catch(()=>{});
    } else {
      navigator.clipboard?.writeText(text).then(()=>showToast("Copied to clipboard! 📋")).catch(()=>showToast("thestreamhub.app"));
    }
  };

  // Load watchlist movies — reset and reload whenever tab opens or watchlist changes
  useEffect(() => {
    if (tab !== "watchlist") return;
    setWlMovies([]);
    if (!watchlist || watchlist.length === 0) { setLoadingWl(false); return; }
    setLoadingWl(true);
    const ids = watchlist.slice(0,40);
    Promise.allSettled(ids.map(async id => {
      try {
        const movie = await tmdbFetch(`/movie/${id}?language=en-US`);
        if (movie?.id) return {...movie, mediaType:"movie"};
      } catch {}
      try {
        const tv = await tmdbFetch(`/tv/${id}?language=en-US`);
        if (tv?.id) return {...tv, mediaType:"tv"};
      } catch {}
      return null;
    })).then(results => {
      setWlMovies(results.map(r=>r.status==="fulfilled"?r.value:null).filter(Boolean));
      setLoadingWl(false);
    }).catch(()=>setLoadingWl(false));
  }, [tab, watchlist.length]); // re-runs whenever watchlist size changes
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:1100,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(8px)",animation:"fadeIn .2s"}}>
      <div onClick={e=>e.stopPropagation()} className="fadeUp" style={{background:"var(--surface)",borderRadius:22,width:"100%",maxWidth:520,maxHeight:"90vh",border:"1px solid var(--border)",boxShadow:"0 40px 80px rgba(0,0,0,.8)",overflow:"hidden",display:"flex",flexDirection:"column"}}>
        ...
        <div style={{background:"linear-gradient(135deg,rgba(139,92,246,.3),rgba(245,158,11,.1))",padding:"24px 24px 20px",position:"relative",flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,marginTop:6,flexWrap:"wrap"}}>
                {tier==="premium"
                  ? <span style={{background:"var(--gold)",color:"#000",fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:99,fontFamily:"var(--font-head)"}}>✦ PREMIUM</span>
                ...
                {streak>=1 && (
                  <button onClick={()=>setShowStreakModal(true)}
                    style={{background:"rgba(245,158,11,.12)",border:"1px solid rgba(245,158,11,.3)",color:"var(--gold)",fontSize:10,fontWeight:800,padding:"3px 10px",borderRadius:99,fontFamily:"var(--font-head)",display:"inline-flex",alignItems:"center",gap:4,cursor:"pointer"}}>
          ...
          {/* Share Profile button — replaces stat boxes */}
          <button onClick={shareProfile}
            style={{width:"100%",marginTop:16,background:"rgba(139,92,246,.12)",border:"1px solid rgba(139,92,246,.3)",borderRadius:12,color:"#C4B5FD",padding:"11px 16px",fontFamily:"var(--font-head)",fontWeight:800,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,transition:"all .2s"}}
        ...
        <div style={{display:"flex",borderBottom:"1px solid var(--border)",flexShrink:0,background:"rgba(0,0,0,.2)"}}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{
                flex:1,background:"none",border:"none",
                color:tab===t.id?"var(--gold)":"rgba(240,240,250,.45)",
                fontFamily:"var(--font-head)",fontWeight:700,fontSize:14,
                padding:"13px 4px",
                borderBottom:tab===t.id?"2.5px solid var(--gold)":"2px solid transparent",
              ...
              {showSubManager ? (
                <div style={{background:"rgba(255,255,255,.03)",border:"1px solid rgba(139,92,246,.2)",borderRadius:14,padding:"16px"}}>
                  ...
                  <SubscriptionManagerPanel
                    userSubs={userSubs}
                    onToggle={async (newSubs)=>{
                      if (onSaveSubs) await onSaveSubs(newSubs);
    ...
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.9)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20,backdropFilter:"blur(8px)",animation:"fadeIn .2s"}}>
      <div onClick={e=>e.stopPropagation()} className="fadeUp" style={{background:"var(--surface)",borderRadius:20,width:"100%",maxWidth:780,maxHeight:"92vh",overflow:"hidden",display:"flex",flexDirection:"column",border:"1px solid var(--border)",boxShadow:"0 40px 80px rgba(0,0,0,.8)"}}>
        ...
        <div style={{overflowY:"auto",flex:1,padding:20}}>
                    ...
                    <div style={{marginBottom:10}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                        {allProviders.rent.map((p,i)=>(
                          <a key={i} href={getPlatformLink(p.provider_name, movie.id, movie.title||movie.name, allProviders.link)} target="_blank" rel="noopener noreferrer"
                            style={{display:"flex",alignItems:"center",gap:6,background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:10,padding:"6px 12px",fontSize:12,fontWeight:700,textDecoration:"none",color:"var(--text)",cursor:"pointer",transition:"all .2s"}}
          ...
          {tab==="reviews" && (
            <div>
              <div style={{background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:"var(--radius)",padding:18,marginBottom:24}}>
...
function UpgradeModal({ onClose, onComplete }) {
  ...
  const PLANS = [
    { id:"monthly",  emoji:"💜", label:"Monthly",  price:"$4.99",  sub:"/month",      badge:null,              desc:"Billed monthly · Cancel anytime" },
    { id:"annual",   emoji:"🌟", label:"Annual",   price:"$24.99", sub:"/year",       badge:"BEST VALUE",      desc:"Just $2.08/mo · Save 58% vs monthly" },
    { id:"lifetime", emoji:"👑", label:"Lifetime", price:"$19.99", sub:"one-time",    badge:"LIMITED OFFER",   desc:"Pay once, keep Premium forever" },
  ];

  const FEATURES = [
    { text:"Unlimited searches",          free:true },
    { text:"🎭 Mood Search",              free:true },
    { text:"🏆 Sports Hub (live scores)", free:true },
    { text:"Watchlist & ratings",         free:true },
    { text:"1 free AI pick",              free:true },
    { text:"Unlimited AI picks (Watch Tonight & For You)", free:false },
    { text:"🚨 Leaving Soon alerts",      free:false },
    { text:"📺 Watch History & Stats",    free:false },
    { text:"💰 AI Cost Report",           free:false },
    { text:"Unlimited watchlist",         free:false },
  ];
      ...
      if (data.url) {
        ...
        if (typeof gtag === "function") {
          gtag("event", "begin_checkout", {
            currency: "USD",
            value: selectedPlan==="monthly" ? 4.99 : selectedPlan==="annual" ? 24.99 : 19.99,
            items: [{ item_name: `StreamHub Premium ${selectedPlan}` }],
          });
        }
        window.location.href = data.url;
      }
      else { alert("Checkout error — please try again."); setLoading(false); }
    } catch(e) { alert("Network error — please try again."); setLoading(false); }
  };

  const plan = PLANS.find(p=>p.id===selectedPlan);

  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.88)",zIndex:1100,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px",backdropFilter:"blur(10px)",animation:"fadeIn .2s",overflowY:"auto"}}>
      <div onClick={e=>e.stopPropagation()} className="fadeUp" style={{background:"var(--surface)",borderRadius:22,width:"100%",maxWidth:500,border:"1px solid var(--border)",boxShadow:"0 40px 80px rgba(0,0,0,.85)",overflow:"hidden",margin:"auto"}}>

        {/* Header */}
        <div style={{padding:"24px 24px 0",display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
          <div>
            <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:22,background:"linear-gradient(90deg,#F59E0B,#f0abfc)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Upgrade to Premium ✦</div>
            <div style={{fontSize:13,color:"var(--muted)",marginTop:3}}>Unlock every feature — pick the plan that fits</div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",color:"var(--muted)",fontSize:20,cursor:"pointer",flexShrink:0,marginLeft:12}}>✕</button>
        </div>
          ...
          {PLANS.map(p=>{
            ...
            return (
              <button key={p.id} onClick={()=>setSelectedPlan(p.id)}
                style={{background:sel?"rgba(245,158,11,.08)":"rgba(255,255,255,.03)",border:`2px solid ${sel?"var(--gold)":"rgba(255,255,255,.1)"}`,borderRadius:14,padding:"13px 16px",display:"flex",alignItems:"center",gap:14,cursor:"pointer",transition:"all .18s",textAlign:"left",position:"relative"}}>
                {p.badge&&<div style={{position:"absolute",top:-9,right:14,background:p.id==="lifetime"?"#8B5CF6":"var(--gold)",color:"#000",fontSize:8,fontWeight:900,padding:"2px 10px",borderRadius:99,fontFamily:"var(--font-head)",whiteSpace:"nowrap",letterSpacing:.5}}>{p.badge}</div>}
                {/* Radio dot */}
                <div style={{width:18,height:18,borderRadius:"50%",border:`2px solid ${sel?"var(--gold)":"rgba(255,255,255,.25)"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  {sel&&<div style={{width:8,height:8,borderRadius:"50%",background:"var(--gold)"}}/>}
                </div>
                <div style={{fontSize:20,flexShrink:0}}>{p.emoji}</div>
                <div style={{flex:1}}>
                  <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:15,color:sel?"var(--gold)":"var(--text)"}}>{p.label}</div>
                  <div style={{fontSize:11,color:"var(--muted)",marginTop:1}}>{p.desc}</div>
                </div>
                <div style={{textAlign:"right",flexShrink:0}}>
                  <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:18,color:sel?"var(--gold)":"var(--text)"}}>{p.price}</div>
        ...
        <div style={{padding:"16px 24px 0"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 80px 80px",gap:"6px 0",background:"rgba(255,255,255,.03)",borderRadius:12,padding:"12px 14px"}}>
            <div style={{fontSize:9,fontWeight:800,color:"var(--muted)",letterSpacing:1,fontFamily:"var(--font-head)",paddingBottom:6}}>FEATURES</div>
            <div style={{fontSize:9,fontWeight:800,color:"var(--muted)",letterSpacing:1,textAlign:"center",paddingBottom:6,fontFamily:"var(--font-head)"}}>FREE</div>
            <div style={{fontSize:9,fontWeight:800,color:"var(--gold)",letterSpacing:1,textAlign:"center",paddingBottom:6,fontFamily:"var(--font-head)"}}>PREMIUM</div>
            {FEATURES.map((f,i)=>(
              <React.Fragment key={i}>
                <div style={{fontSize:11,color:"var(--text)",padding:"4px 0",borderTop:"1px solid rgba(255,255,255,.04)"}}>{f.text}</div>
                <div style={{textAlign:"center",padding:"4px 0",borderTop:"1px solid rgba(255,255,255,.04)",fontSize:13}}>{f.free?"✓":"—"}</div>
                <div style={{textAlign:"center",padding:"4px 0",borderTop:"1px solid rgba(255,255,255,.04)",fontSize:13,color:"var(--gold)"}}>✓</div>
              </React.Fragment>
            ))}
          </div>
        </div>
        ...
        <div style={{padding:"16px 24px 24px"}}>
          <button onClick={handleCheckout} disabled={loading}
            style={{width:"100%",background:loading?"rgba(245,158,11,.5)":"linear-gradient(135deg,#F59E0B,#f59e0b)",border:"none",borderRadius:13,color:"#000",padding:"14px 0",fontFamily:"var(--font-head)",fontWeight:900,fontSize:15,cursor:loading?"default":"pointer",boxShadow:"0 8px 28px rgba(245,158,11,.35)",display:"flex",alignItems:"center",justifyContent:"center",gap:10,transition:"all .2s"}}>
            {loading
              ?<><span style={{display:"inline-block",width:18,height:18,border:"2px solid #000",borderTopColor:"transparent",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>Redirecting to Stripe…</>
              :`${plan.emoji} Get ${plan.label} — ${plan.price} ${plan.id==="lifetime"?"one-time":plan.sub} →`
            }
          </button>
          <div style={{textAlign:"center",fontSize:11,color:"var(--muted)",marginTop:10}}>
            🔒 Secured by <span style={{color:"#6772e5",fontWeight:700}}>Stripe</span>
            {selectedPlan!=="lifetime"&&" · Cancel anytime"}
            {" · No hidden fees"}
          </div>
        </div>
...
// ─── APP FOOTER ───────────────────────────────────────────────────────────────
function AppFooter({ onPrivacy, onTerms, mobile=false, tablet=false }) {
  return (
    <div style={{
      background:"#000",
      borderTop:"1px solid rgba(245,158,11,.15)",
      paddingBottom: mobile ? 96 : tablet ? 120 : 0,
      textAlign:"center",
    }}>
      {/* Banner image — full width, no padding, no border-radius */}
      <img
        src="/banner.jpeg"
        alt="The StreamHub — Your AI Streaming Assistant"
        style={{
          width:"100%",
          display:"block",
          objectFit:"contain",
        ...
        onError={e => {
          ...
          fb.style.cssText = `background:linear-gradient(135deg,#0d0520,#1a0a38);padding:${mobile?"24px 16px":"36px 40px"};display:flex;align-items:center;justify-content:center;gap:${mobile?16:32}px;flex-wrap:wrap`;
          fb.innerHTML = `
            <div style="font-family:var(--font-head);font-weight:900;font-size:${mobile?22:36}px">
              <span style="color:#F59E0B">The Stream</span><span style="color:#8B5CF6">Hub</span>
            </div>
            <div style="color:rgba(245,158,11,.8);font-size:${mobile?10:12}px;letter-spacing:3px;font-weight:700">YOUR AI STREAMING ASSISTANT</div>
            <div style="display:flex;gap:${mobile?12:20}px;font-weight:800;font-size:${mobile?13:18}px">
              <span style="color:#F59E0B">Search</span>
              <span style="color:rgba(255,255,255,.3)">•</span>
              <span style="color:#8B5CF6">Find</span>
              <span style="color:rgba(255,255,255,.3)">•</span>
              <span style="color:#fff">Enjoy</span>
            </div>`;
          e.target.parentElement.insertBefore(fb, e.target.nextSibling);
        }}
      />
      ...
      <div style={{
        ...
        <a href="/privacy.html" target="_blank" rel="noopener" style={{color:"rgba(240,240,250,.35)",fontSize:11,fontFamily:"inherit",textDecoration:"underline"}}>Privacy Policy</a>
        <span style={{color:"rgba(255,255,255,.15)",fontSize:11}}>·</span>
        <a href="/terms.html" target="_blank" rel="noopener" style={{color:"rgba(240,240,250,.35)",fontSize:11,fontFamily:"inherit",textDecoration:"underline"}}>Terms of Service</a>
        <span style={{color:"rgba(255,255,255,.15)",fontSize:11}}>·</span>
        <span style={{fontSize:11,color:"rgba(240,240,250,.25)"}}>© 2026 The StreamHub</span>
        <span style={{color:"rgba(255,255,255,.15)",fontSize:11}}>·</span>
        <span style={{fontSize:10,color:"rgba(240,240,250,.2)"}}>Not affiliated with any streaming service.</span>
      </div>
    </div>
  );
}
...
function MobileBottomNav({ view, setView, watchlist, onProfile, tier }) {
  ...
  return (
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:200,background:"rgba(9,7,15,.98)",borderTop:"1px solid rgba(255,255,255,.08)",display:"flex",alignItems:"center",backdropFilter:"blur(20px)",paddingBottom:"env(safe-area-inset-bottom)"}}>
      {tabs.map(t=>{
        const active = t.isHome ? view==="home" : view===t.id;
        if (t.isHome) return (
          <button key="home" onClick={()=>setView("home")}
            style={{flex:1.2,background:"none",border:"none",padding:"6px 0 6px",display:"flex",flexDirection:"column",alignItems:"center",gap:2,cursor:"pointer",position:"relative"}}>
            <div style={{
              ...
              display:"flex",alignItems:"center",justifyContent:"center",
              fontSize:20,
              boxShadow:active?"0 0 16px rgba(245,158,11,.45)":"none",
              transition:"all .2s",
            }}>🏠</div>
            <span style={{fontSize:9,fontWeight:800,fontFamily:"var(--font-head)",color:active?"#F59E0B":"rgba(240,240,250,.38)",letterSpacing:.3}}>Home</span>
          </button>
        );
        return (
            ...
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:24}}>
                {[["📺",totalWatched,"Total Watched"],["📅",thisMonth,"This Month"],["🗓️",thisYear,"This Year"],["🎬",movies,"Movies"],["📡",shows,"TV Shows"],["⭐",Math.round(totalWatched*1.2),"Hours Est."]].map(([icon,val,label])=>(
                  <div key={label} style={{background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:12,padding:"14px 10px",textAlign:"center"}}>
...
function CostCalculatorModal({ onClose, userSubs, watchHistory, watchlist, userRatings, tier, onUpgrade }) {
  ...
  const generateAIReport = async () => {
    ...
    if (!checkRateLimit("cost_report")) {
      setAiReport({ error:"Please wait a few minutes before generating another report.", summary:"", keep:[], drop:[], tip:"" });
      setLoading(false);
      return;
    }
    try {
...
function MoodSearchModal({ onClose, tier, onUpgrade, onResults }) {
  ...
  const search = async () => {
    if (!mood.trim()) return;
    if (!checkRateLimit("mood_search")) {
      showToast?.("Please wait a moment before searching again 🔮");
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.92)",zIndex:1100,display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:56,paddingBottom:20,paddingLeft:12,paddingRight:12,overflowY:"auto",backdropFilter:"blur(10px)",animation:"fadeIn .2s"}}>
      <div onClick={e=>e.stopPropagation()} className="fadeUp" style={{
        background:"linear-gradient(180deg,#180C00 0%,#0F0700 100%)",
        borderRadius:20,
        width:"100%", maxWidth:640,
        overflow:"hidden",
        display:"flex", flexDirection:"column",
        border:"1px solid rgba(251,146,60,.4)",
        boxShadow:"0 20px 80px rgba(251,146,60,.35)",
      }}>
        {/* Header */}
        <div style={{
          padding:"20px 20px 16px",
          background:"linear-gradient(135deg,rgba(251,146,60,.2),rgba(194,65,12,.1))",
          borderBottom:"1px solid rgba(251,146,60,.2)",
          position:"relative",
        }}>
          <div style={{position:"absolute",top:-40,left:-40,width:150,height:150,borderRadius:"50%",background:"rgba(251,146,60,.2)",filter:"blur(50px)",pointerEvents:"none"}}/>
          <div style={{position:"absolute",top:-40,right:-40,width:150,height:150,borderRadius:"50%",background:"rgba(194,65,12,.15)",filter:"blur(50px)",pointerEvents:"none"}}/>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6,position:"relative"}}>
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              <div style={{
                ...
                display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:20,boxShadow:"0 4px 20px rgba(251,146,60,.5)",
              }}>🎭</div>
              <div>
                <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontSize:24,letterSpacing:1.5,display:"flex",alignItems:"center",gap:8,color:"#fdba74"}}>
                  Mood Search
                ...
                <div style={{fontSize:12,color:"rgba(253,186,116,.8)"}}>Describe any vibe — AI finds the perfect match</div>
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}>
              <div style={{background:"rgba(251,146,60,.15)",border:"1px solid rgba(251,146,60,.3)",borderRadius:99,padding:"3px 10px",fontSize:10,fontWeight:700,color:"#fb923c"}}>
                🎭 Free
              </div>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8,position:"relative"}}>
            {['"scary but not gory"','"fun date night"','"mind-bending sci-fi"','"feel-good comedy"'].map(ex=>(
              <button key={ex} onClick={()=>setMood(ex.replace(/"/g,""))}
                style={{background:"rgba(251,146,60,.12)",border:"1px solid rgba(251,146,60,.3)",borderRadius:99,color:"#fdba74",padding:"4px 12px",fontSize:11,cursor:"pointer",fontStyle:"italic"}}>
                {ex}
              </button>
            ))}
          </div>
        ...
        <div style={{overflowY:"auto",padding:"16px 20px 24px",flex:1}}>
          ...
          <div style={{position:"relative",marginBottom:16}}>
            ...
            <input
              ...
              autoFocus
              style={{
                width:"100%", background:"rgba(251,146,60,.06)",
            ...
            <button onClick={search} disabled={loading||!mood.trim()}
              style={{
                ...
                background:mood.trim()?"linear-gradient(135deg,#c2410c,#fb923c)":"rgba(255,255,255,.1)",
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                {moods.map(m=>(
                  <button key={m} onClick={()=>setMood(m)}
                    style={{
                      background:mood===m?"rgba(251,146,60,.25)":"rgba(255,255,255,.05)",
                      border:`1px solid ${mood===m?"rgba(251,146,60,.7)":"rgba(255,255,255,.1)"}`,
                ...
                <div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    {result.items?.map((item,i)=>(
                      <div key={i} style={{background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:14,padding:"12px 14px",animation:`fadeUp .3s ${i*0.06}s both`,transition:"border-color .2s"}}
                        onMouseEnter={e=>e.currentTarget.style.borderColor="rgba(251,146,60,.5)"}
                        onMouseLeave={e=>e.currentTarget.style.borderColor="rgba(251,146,60,.2)"}>
                        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10,marginBottom:5}}>
                          <div style={{flex:1}}>
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap"}}>
                              ...
                              {item.platform&&<span style={{background:"rgba(251,146,60,.2)",borderRadius:6,padding:"2px 7px",fontSize:10,color:"#c4b5fd",fontWeight:700}}>{item.platform}</span>}
                            </div>
                          </div>
                          <div style={{display:"flex",alignItems:"center",gap:6}}>
                            <button onClick={()=>{onResults(item.tmdb_search||item.title);onClose();}}
                              style={{background:"linear-gradient(135deg,#c2410c,#fb923c)",border:"none",borderRadius:10,color:"#fff",padding:"7px 12px",fontSize:11,fontWeight:800,cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>
                              Search →
                            </button>
                            <button onClick={()=>{
...
function InstallPrompt({ onDismiss }) {
  ...
  return (
    <div style={{
      ...
      {isIOS ? (
        <div style={{fontSize:12,color:"rgba(240,240,250,.6)",lineHeight:1.7,background:"rgba(251,146,60,.07)",border:"1px solid rgba(251,146,60,.2)",borderRadius:10,padding:"10px 12px"}}>
...
{"items":[{"title":"...","year":2023,"type":"movie or tv","reason":"personalized reason based on their taste in one sentence","genre":"...","tmdb_search":"exact title"}]}`;
      ...
      if (!checkRateLimit("personalized")) {
        setLoading(false);
        return;
      }
      const res = await fetch("/api/ai", {
  ...
  useEffect(() => { 
    // Don't auto-fire on mount — user must click "Get Recommendations"
    // This was burning credits on every page load
  }, []);
...
export default function StreamHub() {
  ...
  const SEARCH_PLACEHOLDERS = [
    ...
    "Try \"something like Interstellar\"…",
    "Try \"best sci-fi of the 90s\"…",
    "Try \"short series to binge tonight\"…",
    "Try \"action movie, no commitment\"…",
    "Try \"feel-good comedy for the family\"…",
    "Try \"mind-bending thriller\"…",
  ];
  const [placeholderIdx, setPlaceholderIdx] = useState(0);
  useEffect(()=>{
    if (search.trim()) return;
    const iv = setInterval(()=>setPlaceholderIdx(i=>(i+1)%SEARCH_PLACEHOLDERS.length), 3200);
    return ()=>clearInterval(iv);
  }, [search]);
  ...
  const [planType, setPlanType] = useState("monthly"); // "monthly" | "annual" | "lifetime"
  ...
  const [predCelebration, setPredCelebration] = useState(null);
  const [predMiss, setPredMiss] = useState(null);
  const handlePredResult = (result) => {
    const shownKey = `sh_pred_shown_${result.gameId||Date.now()}`;
    if (localStorage.getItem(shownKey)) return; // already shown
    localStorage.setItem(shownKey, "1");
    if (result.correct) {
      setPredCelebration(result);
    } else {
      setPredMiss({streak: result.streak});
    }
  };
      ...
      supabase.auth.getSession().then(({data:{session}})=>{
        if (session?.user) {
          supabase.from("profiles")
            .update({ favorite_teams: JSON.stringify(updated) })
  ...
  useEffect(() => {
    ...
    if (params.get("success") === "true") {
      const paidPlan = params.get("plan") || "monthly"; // monthly | annual | lifetime
      // Clean the URL
      ...
      const upgradeUser = async () => {
        ...
        if (session?.user) {
          await supabase.from("profiles").update({ tier:"premium", plan_type:paidPlan }).eq("id", session.user.id);
          setTier("premium");
          setPlanType(paidPlan);
          // ── GA4 conversion event ──────────────────────────────────
          if (typeof gtag === "function") {
            const planValue = paidPlan==="monthly" ? 4.99 : paidPlan==="annual" ? 24.99 : 19.99;
            gtag("event", "purchase", {
              transaction_id: `${session.user.id}_${Date.now()}`,
              currency: "USD",
              value: planValue,
              items: [{ item_name: `StreamHub Premium ${paidPlan}`, price: planValue, quantity: 1 }],
            });
          }
          const emoji = paidPlan==="lifetime"?"👑":paidPlan==="annual"?"🌟":"💜";
          showToast(`${emoji} Welcome to Premium ${paidPlan.charAt(0).toUpperCase()+paidPlan.slice(1)}! All features unlocked.`);
        } else {
          // Store flag so we can upgrade after they sign in
          localStorage.setItem("streamhub_pending_upgrade", "true");
          localStorage.setItem("streamhub_pending_plan", paidPlan);
    ...
    if (localStorage.getItem("streamhub_pending_upgrade") === "true") {
      const tryPendingUpgrade = async () => {
        const { data:{ session } } = await supabase.auth.getSession();
        if (session?.user) {
          const pendingPlan = localStorage.getItem("streamhub_pending_plan") || "monthly";
          await supabase.from("profiles").update({ tier:"premium", plan_type:pendingPlan }).eq("id", session.user.id);
          setTier("premium");
          setPlanType(pendingPlan);
          localStorage.removeItem("streamhub_pending_upgrade");
          localStorage.removeItem("streamhub_pending_plan");
  ...
  const loadUserData = async (u) => {
    ...
    // Load favorite teams — merge Supabase + localStorage so changes survive refresh
    if (prof.favorite_teams) {
      try {
        ...
        if (ft && typeof ft === "object" && Object.keys(ft).length > 0) {
          const local = JSON.parse(localStorage.getItem("streamhub_fav_teams")||"{}");
          const merged = { ...local, ...ft };
    ...
    // ── Load prediction stats from Supabase (cross-device) ──────────────────
    if (prof.pred_stats) {
      try {
        const ps = typeof prof.pred_stats==="string" ? JSON.parse(prof.pred_stats) : prof.pred_stats;
        localStorage.setItem("sh_pred_stats", JSON.stringify(ps));
      } catch(e) {}
    }

    // ── Load + update login streak from Supabase (cross-device) ─────────────
    const cloudStreak = prof.login_streak || null;
    const newStreak = getStreak(cloudStreak);
    const streakData = {lastVisit:new Date().toDateString(), streak:newStreak};
    localStorage.setItem("streamhub_streak", JSON.stringify(streakData));
    // Save updated streak back to Supabase
    supabase.from("profiles").update({ login_streak: JSON.stringify(streakData) }).eq("id",u.id).then(()=>{}).catch?.(()=>{});
  };
  ...
  useEffect(() => {
    ...
    // Watchlist: fetch each saved ID directly from TMDB
    if (view === "watchlist") {
      if (!watchlist || watchlist.length === 0) { setMovies([]); setLoading(false); return; }
      setLoading(true);
      Promise.allSettled(watchlist.slice(0,40).map(async id => {
        try {
          const m = await tmdbFetch(`/movie/${id}?language=en-US`);
          if (m?.id) return {...m, mediaType:"movie", providers:[], category:"watchlist"};
        } catch {}
        try {
          const t = await tmdbFetch(`/tv/${id}?language=en-US`);
          if (t?.id) return {...t, mediaType:"tv", providers:[], category:"watchlist"};
        } catch {}
        return null;
      })).then(results => {
        setMovies(results.map(r=>r.status==="fulfilled"?r.value:null).filter(Boolean));
        setLoading(false);
      }).catch(()=>setLoading(false));
      return;
    }
  ...
  useEffect(() => {
    ...
    // Don't run movie search when on sports view — search is used as sportQuery there
    if (view === "sports") { setSearchResults([]); return; }
    ...
    searchTimer.current = setTimeout(async () => {
      ...
      setSearching(true);
      track("search", { search_term: q.trim(), is_mood_search: !!isGenreSearch(q) });
      try {
        const genreConfig = isGenreSearch(q);
        if (genreConfig) {
          // Genre/mood search
  ...
  // Coerce search to string defensively — MoodSearch/AI can occasionally set non-string values
  ...
  if (isMobile) return (
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)",paddingBottom:0}}>
        ...
        <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(9,7,15,.97)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(245,158,11,.1)",paddingTop:"env(safe-area-inset-top)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",padding:"10px 14px 8px",gap:10}}>
            ...
            <button onClick={()=>{setView("stats");setSearch("");}}
              style={{background:view==="stats"?"rgba(16,185,129,.15)":"rgba(255,255,255,.06)",border:`1px solid ${view==="stats"?"rgba(16,185,129,.4)":"rgba(255,255,255,.1)"}`,borderRadius:10,color:view==="stats"?"#10B981":"var(--muted)",padding:"4px 8px",minWidth:36,height:36,fontSize:15,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,flexShrink:0}}>
              <span>📊</span><span style={{fontSize:8,fontWeight:800,lineHeight:1}}>Stats</span>
            </button>
            {/* Saved shortcut */}
            <button onClick={()=>{setView("watchlist");setSearch("");}}
              style={{background:view==="watchlist"?"rgba(239,68,68,.15)":"rgba(255,255,255,.06)",border:`1px solid ${view==="watchlist"?"rgba(239,68,68,.4)":"rgba(255,255,255,.1)"}`,borderRadius:10,color:view==="watchlist"?"#ef4444":"var(--muted)",padding:"4px 8px",minWidth:36,height:36,fontSize:15,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:1,flexShrink:0}}>
              <span>❤️</span><span style={{fontSize:8,fontWeight:800,lineHeight:1}}>Saved</span>
            </button>
            {tier==="premium"
              ?planType==="lifetime"?<span onClick={()=>setShowReferral(true)} style={{background:"linear-gradient(135deg,#F59E0B,#f59e0b)",color:"#000",fontSize:9,fontWeight:900,padding:"3px 8px",borderRadius:99,fontFamily:"var(--font-head)",flexShrink:0,cursor:"pointer"}}>👑 Lifetime</span>:planType==="annual"?<span onClick={()=>setShowReferral(true)} style={{background:"linear-gradient(135deg,#8B5CF6,#7C3AED)",color:"#fff",fontSize:9,fontWeight:900,padding:"3px 8px",borderRadius:99,fontFamily:"var(--font-head)",flexShrink:0,cursor:"pointer"}}>🌟 Annual</span>:<span onClick={()=>setShowReferral(true)} style={{background:"linear-gradient(135deg,#EC4899,#be185d)",color:"#fff",fontSize:9,fontWeight:900,padding:"3px 8px",borderRadius:99,fontFamily:"var(--font-head)",flexShrink:0,cursor:"pointer"}}>💜 Monthly</span>
        ...
        {!user && view==="home" && !safeSearch.trim() && <WelcomeBanner />}

        {/* 🎭 AI BRAND BANNER — mobile, right under search bar */}
        {!safeSearch.trim() && view==="home" && (
          <div style={{
            ...
            <div style={{
              fontFamily:"'Bebas Neue',var(--font-head)", fontWeight:400,
              fontSize:28, letterSpacing:2, marginBottom:12,
        ...
        {/* Mood Search + Sports Hub — side-by-side cards, home view only */}
        {view==="home"&&!safeSearch.trim()&&(<>
        <div style={{padding:"0 14px 12px",display:"flex",flexDirection:"column",gap:8}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div onClick={()=>setShowMoodSearch(true)}
              style={{
                background:"linear-gradient(135deg,#140a00,#2e1800,#0d0600)",
                border:"1.5px solid rgba(251,146,60,.55)",
                borderRadius:16, padding:"14px 12px",
                cursor:"pointer", position:"relative", overflow:"hidden",
                boxShadow:"0 0 20px rgba(251,146,60,.2)",
                display:"flex", flexDirection:"column", gap:8,
                textAlign:"center", alignItems:"center", transition:"all .2s",
              }}
              onTouchStart={e=>e.currentTarget.style.transform="scale(.97)"}
              onTouchEnd={e=>e.currentTarget.style.transform="scale(1)"}>
              <div style={{position:"absolute",top:-20,right:-20,width:90,height:90,borderRadius:"50%",background:"rgba(251,146,60,.18)",filter:"blur(25px)",pointerEvents:"none"}}/>
              <div style={{width:40,height:40,borderRadius:12,background:"linear-gradient(135deg,#c2410c,#fb923c)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0,boxShadow:"0 4px 14px rgba(251,146,60,.5)"}}>🎭</div>
              <div style={{position:"relative",textAlign:"center"}}>
                <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontSize:20,letterSpacing:1.5,background:"linear-gradient(90deg,#fdba74,#fff7ed)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1}}>Mood Search</div>
                <div style={{fontSize:10,color:"rgba(253,186,116,.75)",marginTop:3,lineHeight:1.3}}>Describe a vibe, AI finds the match</div>
              </div>
              <span style={{background:"rgba(251,146,60,.2)",border:"1px solid rgba(251,146,60,.4)",borderRadius:99,padding:"2px 8px",fontSize:9,fontWeight:700,color:"#fb923c"}}>Free ✓</span>
            </div>

            {/* Sports Hub card */}
            <button onClick={()=>{setView("sports");setSearch("");setShowAllGames(false);window.scrollTo({top:0,behavior:"smooth"});}}
              style={{
                background:"linear-gradient(135deg,rgba(7,15,7,.98),rgba(10,30,15,.98),rgba(5,20,10,.98))",
              ...
              onTouchStart={e=>e.currentTarget.style.transform="scale(.97)"}
              onTouchEnd={e=>e.currentTarget.style.transform="scale(1)"}>
              <div style={{position:"absolute",top:-20,right:-20,width:90,height:90,borderRadius:"50%",background:"rgba(16,185,129,.15)",filter:"blur(25px)",pointerEvents:"none"}}/>
              <div style={{position:"relative",width:40,height:40,borderRadius:12,background:"rgba(16,185,129,.15)",border:"1px solid rgba(16,185,129,.3)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              ...
              <div style={{position:"relative",textAlign:"center"}}>
                <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontSize:20,letterSpacing:1.5,color:"#fff",lineHeight:1}}>Sports Hub</div>
                <div style={{fontSize:10,color:"rgba(16,185,129,.8)",marginTop:3,lineHeight:1.3}}>Live scores · World Cup 🔴</div>
              </div>
              <span style={{background:"rgba(239,68,68,.15)",border:"1px solid rgba(239,68,68,.35)",borderRadius:99,padding:"2px 8px",fontSize:9,fontWeight:800,color:"#ef4444",letterSpacing:.5}}>LIVE</span>
            </button>

          </div>

          {/* Today's Pick */}
          {featuredRows.trending?.length>0&&(
            <DailyPickBanner
        ...
        {view==="home"&&!safeSearch.trim() ? (
          ...
          <div style={{padding:"12px 14px",overflowY:"auto",flex:1}}>
            {!safeSearch.trim() ? (
              <>
                {/* My Teams — full width with Predictions pill embedded */}
                <div style={{marginBottom:8}}>
                  <FavoriteTeamsBanner favoriteTeams={favoriteTeams} compact={true}/>
                </div>
              ...
              <>
                <button onClick={()=>setSearch("")} style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(16,185,129,.08)",border:"1.5px solid rgba(16,185,129,.35)",borderRadius:99,color:"#10B981",padding:"6px 14px",fontSize:12,fontWeight:700,cursor:"pointer",marginBottom:14,boxShadow:"0 0 12px rgba(16,185,129,.2)",transition:"all .2s"}}>← Back to Sports Hub</button>
                {showAllGames ? <AllGamesSection onBack={()=>setShowAllGames(false)}/>
                  : safeSearch==="soccer_hub" ? <SoccerHub onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                  : safeSearch.toLowerCase().includes("olympic") ? <OlympicsPlaceholder/>
                  : <><LiveSportsSection sportQuery={safeSearch} favoriteTeams={favoriteTeams} onToggleFavorite={toggleFavoriteTeam} user={user} showToast={showToast} onPredResult={handlePredResult}/><SportMovieBridge activeSport={safeSearch} onSelect={handleSelectMovie}/></>}
              </>
            )}
          </div>
        ) : view==="stats" ? (
          <div style={{padding:"14px 14px 100px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:2}}>
              <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:20}}>📊 My Stats</div>
              <button onClick={()=>{
                const text = `📊 My StreamHub Stats\n\n❤️ ${watchlist.length} titles saved · ⭐ ${Object.keys(userRatings).length} rated\n\nTrack your streaming life at thestreamhub.app`;
                navigator.share?navigator.share({title:"My StreamHub Stats",text,url:"https://thestreamhub.app"}).catch(()=>{}):navigator.clipboard?.writeText(text).then(()=>showToast("Copied! 📋"));
              }} style={{background:"rgba(139,92,246,.1)",border:"1px solid rgba(139,92,246,.3)",borderRadius:10,color:"#C4B5FD",padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>📤 Share</button>
            </div>
          ...
          /* Regular grid — including watchlist view */
          <div style={{padding:"12px 14px 100px"}}>
            {view==="watchlist" && (
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                <div style={{fontFamily:"var(--font-head)",fontWeight:900,fontSize:20}}>❤️ Saved <span style={{fontSize:14,color:"var(--muted)",fontWeight:600}}>({watchlist.length})</span></div>
                <button onClick={()=>{
                  const text = `❤️ My StreamHub Watchlist\n\nI have ${watchlist.length} titles saved on StreamHub — the AI streaming guide for Netflix, Hulu, Disney+, Max and more.\n\nFind what to watch at thestreamhub.app`;
                  navigator.share?navigator.share({title:"My Watchlist",text,url:"https://thestreamhub.app"}).catch(()=>{}):navigator.clipboard?.writeText(text).then(()=>showToast("Copied! 📋"));
                }} style={{background:"rgba(239,68,68,.1)",border:"1px solid rgba(239,68,68,.3)",borderRadius:10,color:"#f87171",padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>📤 Share</button>
              </div>
            )}
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>
            {loading&&!search
              ...
              : filtered.length===0
                ? <div style={{gridColumn:"1/-1",textAlign:"center",color:"var(--muted)",padding:"60px 0",fontSize:15}}>{view==="watchlist"?"Your watchlist is empty. Tap ♡ on any title to save it!":"No results found."}</div>
      ...
      {/* Footer — in scrollable content, visible above bottom nav */}
      <AppFooter mobile={true} onPrivacy={()=>setShowPrivacy(true)} onTerms={()=>setShowTerms(true)} />
      <MobileBottomNav view={view} setView={v=>{handleSetView(v);setSearch("");}} watchlist={watchlist} tier={tier} onProfile={()=>user?setShowProfile(true):setShowAuth(true)} />
        ...
        {/* Advanced Stats Section */}
                ...
                {/* Spacer so content scrolls fully above bottom nav */}
        <div style={{height:80}} />
      ...
      {showReferral&&<ReferralModal onClose={()=>setShowReferral(false)} user={user}/>}
      ...
      {predMiss&&<PredictionMissModal streak={predMiss.streak} onClose={()=>setPredMiss(null)}/>}
      <CookieConsent/>
      <Analytics />
    </>
  );
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)",paddingBottom:72}}>
        ...
        <header style={{position:"sticky",top:0,zIndex:100,background:"rgba(9,7,15,.97)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(245,158,11,.15)",paddingTop:"env(safe-area-inset-top)"}}>
          <div style={{display:"flex",alignItems:"center",padding:"10px 20px",gap:12,height:64}}>
            ...
            <div style={{flex:1,position:"relative",maxWidth:380}}>
              ...
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={SEARCH_PLACEHOLDERS[placeholderIdx]}
            ...
            <div style={{display:"flex",gap:8,marginLeft:"auto"}}>
              {tier==="premium"
                ?planType==="lifetime"?<span onClick={()=>setShowReferral(true)} style={{background:"linear-gradient(135deg,#F59E0B,#f59e0b)",color:"#000",fontSize:11,fontWeight:900,padding:"5px 13px",borderRadius:99,fontFamily:"var(--font-head)",cursor:"pointer",whiteSpace:"nowrap"}}>👑 Lifetime</span>:planType==="annual"?<span onClick={()=>setShowReferral(true)} style={{background:"linear-gradient(135deg,#8B5CF6,#7C3AED)",color:"#fff",fontSize:11,fontWeight:900,padding:"5px 13px",borderRadius:99,fontFamily:"var(--font-head)",cursor:"pointer",whiteSpace:"nowrap"}}>🌟 Annual</span>:<span onClick={()=>setShowReferral(true)} style={{background:"linear-gradient(135deg,#EC4899,#be185d)",color:"#fff",fontSize:11,fontWeight:900,padding:"5px 13px",borderRadius:99,fontFamily:"var(--font-head)",cursor:"pointer",whiteSpace:"nowrap"}}>💜 Monthly</span>
        ...
        {view==="home"&&!safeSearch.trim()&&(
          <div style={{
            ...
            <div style={{textAlign:"center"}}>
              <div style={{
                fontFamily:"'Bebas Neue',var(--font-head)", fontWeight:400,
                fontSize:36, letterSpacing:2.5, marginBottom:12, lineHeight:1,
                background:"linear-gradient(90deg,#C4B5FD,#E9D5FF,#F59E0B,#C4B5FD)",
                backgroundSize:"200% auto",
                WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent",
                animation:"gradientShift 3s linear infinite",
              ...
              {/* Mood Search now shown in side-by-side grid below hero */}
        ...
        <div style={{padding:"20px 20px 120px"}}>
          ...
          {view==="home"&&!safeSearch.trim()&&<div style={{marginBottom:24}}>
            {/* Mood Search + Sports Hub — side-by-side on tablet */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
              {/* Mood Search */}
              <div onClick={()=>setShowMoodSearch(true)}
                style={{background:"linear-gradient(135deg,#140a00,#2e1800,#0d0600)",border:"1.5px solid rgba(251,146,60,.55)",borderRadius:16,padding:"16px 14px",cursor:"pointer",position:"relative",overflow:"hidden",boxShadow:"0 0 20px rgba(251,146,60,.2)",display:"flex",flexDirection:"column",gap:10,transition:"all .2s"}}
                onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.01)";e.currentTarget.style.borderColor="rgba(251,146,60,.9)";}}
                onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.borderColor="rgba(251,146,60,.55)";}}>
                <div style={{position:"absolute",top:-20,right:-20,width:100,height:100,borderRadius:"50%",background:"rgba(251,146,60,.18)",filter:"blur(28px)",pointerEvents:"none"}}/>
                <div style={{width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#c2410c,#fb923c)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0,boxShadow:"0 4px 14px rgba(251,146,60,.5)"}}>🎭</div>
                <div style={{position:"relative"}}>
                  <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontSize:22,letterSpacing:1.5,background:"linear-gradient(90deg,#fdba74,#fff7ed)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1}}>Mood Search</div>
                  <div style={{fontSize:11,color:"rgba(253,186,116,.75)",marginTop:3,lineHeight:1.3}}>Describe a vibe, AI finds the match</div>
                </div>
                <span style={{background:"rgba(251,146,60,.2)",border:"1px solid rgba(251,146,60,.4)",borderRadius:99,padding:"2px 10px",fontSize:9,fontWeight:700,color:"#fb923c",alignSelf:"flex-start"}}>Free ✓</span>
              </div>
              {/* Sports Hub */}
              <button onClick={()=>{setView("sports");setSearch("");setShowAllGames(false);window.scrollTo({top:0,behavior:"smooth"});}}
                style={{background:"linear-gradient(135deg,rgba(7,15,7,.98),rgba(10,30,15,.98),rgba(5,20,10,.98))",border:"1.5px solid rgba(16,185,129,.5)",borderRadius:16,padding:"16px 14px",cursor:"pointer",position:"relative",overflow:"hidden",boxShadow:"0 0 20px rgba(16,185,129,.2)",display:"flex",flexDirection:"column",gap:10,textAlign:"left",transition:"all .2s"}}
                onMouseEnter={e=>{e.currentTarget.style.transform="scale(1.01)";e.currentTarget.style.borderColor="rgba(16,185,129,.9)";}}
                onMouseLeave={e=>{e.currentTarget.style.transform="scale(1)";e.currentTarget.style.borderColor="rgba(16,185,129,.5)";}}>
                <div style={{position:"absolute",top:-20,right:-20,width:100,height:100,borderRadius:"50%",background:"rgba(16,185,129,.15)",filter:"blur(28px)",pointerEvents:"none"}}/>
                ...
                <div style={{position:"relative"}}>
                  <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontSize:22,letterSpacing:1.5,color:"#fff",lineHeight:1}}>Sports Hub</div>
                  <div style={{fontSize:11,color:"rgba(16,185,129,.8)",marginTop:3,lineHeight:1.3}}>Live scores · World Cup 🔴</div>
                </div>
                <span style={{background:"rgba(239,68,68,.15)",border:"1px solid rgba(239,68,68,.35)",borderRadius:99,padding:"2px 10px",fontSize:9,fontWeight:800,color:"#ef4444",letterSpacing:.5,alignSelf:"flex-start"}}>LIVE</span>
              </button>
            </div>
            {/* Today's Pick — tablet */}
            {featuredRows.trending?.length>0&&<DailyPickBanner
              movie={(featuredRows.trending||[]).filter(m=>(m.providers||[]).length>0)[new Date().getDay()%Math.max(1,(featuredRows.trending||[]).filter(m=>(m.providers||[]).length>0).slice(0,8).length)]||featuredRows.topRated?.[0]}
              onSelect={handleSelectMovie}
              onShare={(m)=>handleShareMovie(m)}
            />}
            ...
            <div>
              {!safeSearch.trim() ? (
                <>
                  {/* My Teams — full width with Predictions pill embedded */}
                  <div style={{marginBottom:10}}>
                    <FavoriteTeamsBanner favoriteTeams={favoriteTeams}/>
                  </div>
                  <SportCategoryGrid onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                  <SportsNewsTicker/>
                </>
                ...
                <>
                  <button onClick={()=>setSearch("")} style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(16,185,129,.08)",border:"1.5px solid rgba(16,185,129,.35)",borderRadius:99,color:"#10B981",padding:"6px 14px",fontSize:12,fontWeight:700,cursor:"pointer",marginBottom:16,boxShadow:"0 0 12px rgba(16,185,129,.2)",transition:"all .2s"}}>← Back to Sports Hub</button>
                  {showAllGames ? <AllGamesSection onBack={()=>setShowAllGames(false)}/>
                    : safeSearch==="soccer_hub" ? <SoccerHub onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                    : safeSearch.toLowerCase().includes("olympic") ? <OlympicsPlaceholder/>
                    : <><LiveSportsSection sportQuery={safeSearch} favoriteTeams={favoriteTeams} onToggleFavorite={toggleFavoriteTeam} user={user} showToast={showToast} onPredResult={handlePredResult} cols={3}/><SportMovieBridge activeSport={safeSearch} onSelect={handleSelectMovie}/></>}
                </>
              )}
            </div>
          ) : view==="stats" ? (
            ...
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:18}}>
                  {view==="watchlist" ? "❤️ Saved" : safeSearch.trim() ? (searching?"Searching…":`${searchResults.length} results for "${search}"`) : CATEGORY_TABS.find(t=>t.id===view)?.icon+" "+CATEGORY_TABS.find(t=>t.id===view)?.label}
                  {!search&&!loading&&<span style={{fontWeight:400,fontSize:14,color:"var(--muted)",marginLeft:10}}>{filtered.length} titles</span>}
                </div>
                {view==="watchlist"&&<button onClick={()=>{
                  const text = `❤️ My StreamHub Watchlist\n\nI have ${watchlist.length} titles saved on StreamHub — the AI streaming guide for Netflix, Hulu, Disney+, Max and more.\n\nFind what to watch at thestreamhub.app`;
                  navigator.share?navigator.share({title:"My Watchlist",text,url:"https://thestreamhub.app"}).catch(()=>{}):navigator.clipboard?.writeText(text).then(()=>showToast("Copied! 📋"));
                }} style={{background:"rgba(239,68,68,.1)",border:"1px solid rgba(239,68,68,.3)",borderRadius:10,color:"#f87171",padding:"6px 12px",fontSize:12,fontWeight:700,cursor:"pointer"}}>📤 Share</button>}
              </div>
        ...
        {/* Footer — tablet */}
        <AppFooter tablet={true} onPrivacy={()=>setShowPrivacy(true)} onTerms={()=>setShowTerms(true)} />
        {/* Tablet Bottom Nav */}
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)"}}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{position:"absolute",left:"50%",top:"50%",transform:"translate(-50%,-50%)",textAlign:"center",width:"60%",pointerEvents:"none"}}>
                <div style={{
                  fontFamily:"'Bebas Neue',var(--font-head)", fontWeight:400,
                  fontSize:42, letterSpacing:3, marginBottom:14,
                  background:"linear-gradient(90deg,#C4B5FD,#E9D5FF,#F59E0B,#C4B5FD)",
                  backgroundSize:"200% auto",
                  WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent",
                  animation:"gradientShift 3s linear infinite",
        ...
        {view==="home"&&!safeSearch.trim()&&(
          <div style={{padding:"0 24px 20px",maxWidth:1440,margin:"0 auto"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              ...
              <div onClick={()=>setShowMoodSearch(true)}
                style={{
                  background:"linear-gradient(135deg,#140a00 0%,#2e1800 60%,#0d0600 100%)",
                  border:"1.5px solid rgba(251,146,60,.6)",
                  borderRadius:18, padding:"20px 24px",
                  cursor:"pointer", position:"relative", overflow:"hidden",
                  boxShadow:"0 8px 32px rgba(251,146,60,.3), 0 0 60px rgba(251,146,60,.1)",
                  transition:"all .25s",
                }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(251,146,60,.9)";e.currentTarget.style.boxShadow="0 12px 48px rgba(251,146,60,.55), 0 0 80px rgba(251,146,60,.2)";e.currentTarget.style.transform="translateY(-2px)";}}
                ...
                <div style={{position:"absolute",top:-40,right:-40,width:200,height:200,borderRadius:"50%",background:"rgba(251,146,60,.22)",filter:"blur(55px)",pointerEvents:"none"}}/>
                ...
                <div style={{position:"relative",display:"flex",alignItems:"flex-start",gap:16}}>
                  <div style={{width:52,height:52,borderRadius:14,background:"linear-gradient(135deg,#c2410c,#fb923c)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,flexShrink:0,boxShadow:"0 6px 20px rgba(139,92,246,.5)"}}>🎭</div>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                      <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontSize:26,letterSpacing:1.5,background:"linear-gradient(90deg,#fdba74,#fff7ed)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1}}>Mood Search</div>
                    ...
                    <div style={{fontSize:13,color:"rgba(253,186,116,.8)",lineHeight:1.6,marginBottom:14}}>
                      Describe your perfect movie night — AI finds the <em>exact</em> match. No scrolling, no browsing. Just vibes.
                      <div style={{marginTop:8}}>
                        <span style={{background:"rgba(251,146,60,.15)",border:"1px solid rgba(251,146,60,.3)",borderRadius:8,padding:"3px 10px",fontSize:11,fontWeight:700,color:"#fb923c"}}>🎭 Free for all users</span>
                      </div>
                    </div>
                    <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(251,146,60,.25)",border:"1px solid rgba(251,146,60,.6)",borderRadius:99,padding:"7px 16px",fontSize:12,fontWeight:700,color:"#fdba74"}}>
                      ✦ Try Mood Search →
                    </div>
                  </div>
                </div>
              ...
              {/* Sports Hub card */}
              <div onClick={()=>{setView("sports");setSearch("");setShowAllGames(false);window.scrollTo({top:0,behavior:"smooth"});}}
                style={{
                ...
                <div style={{position:"relative",display:"flex",alignItems:"flex-start",gap:16}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                      <div style={{fontFamily:"'Bebas Neue',var(--font-head)",fontSize:26,letterSpacing:1.5,background:"linear-gradient(90deg,#6ee7b7,#34d399)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:1}}>Sports Hub</div>
        ...
        <div style={{display:"flex",padding:`${(view==="home"&&!safeSearch.trim()||view==="trending"&&!safeSearch.trim())?"0":"20px"} 24px 20px`,gap:20,maxWidth:1440,margin:"0 auto"}}>
          {/* Main — full width on non-home views */}
            ...
            {view==="home"&&!safeSearch.trim() ? (
              ...
              <div>
                {!safeSearch.trim() ? (
                  ...
                  <>
                    <button onClick={()=>{setSearch("");setShowAllGames(false);}} style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(16,185,129,.08)",border:"1.5px solid rgba(16,185,129,.35)",borderRadius:99,color:"#10B981",padding:"6px 14px",fontSize:12,fontWeight:700,cursor:"pointer",marginBottom:16,boxShadow:"0 0 12px rgba(16,185,129,.2)",transition:"all .2s"}}>← Back to Sports Hub</button>
                    {showAllGames ? <AllGamesSection onBack={()=>setShowAllGames(false)}/>
                      : safeSearch==="soccer_hub" ? <SoccerHub onSearch={handleSportSearch} favoriteTeams={favoriteTeams}/>
                      : safeSearch.toLowerCase().includes("olympic") ? <OlympicsPlaceholder/>
                      : <LiveSportsSection sportQuery={safeSearch} favoriteTeams={favoriteTeams} onToggleFavorite={toggleFavoriteTeam} user={user} showToast={showToast} onPredResult={handlePredResult} cols={4}/>}
                  </>
                )}
              </div>
            ) : view==="stats" ? (
              ...
              <>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                  <div style={{fontFamily:"var(--font-head)",fontWeight:800,fontSize:18}}>
                    {view==="watchlist" ? "❤️ Saved" : safeSearch.trim()
                      ? searching?"Searching…":`${searchResults.length} results for "${search}"`
          ...
          {/* Right Sidebar — only on home view */}
          {view==="home"&&!safeSearch.trim()&&<aside style={{width:220,flexShrink:0}}>
            {!user&&(
        ...
        {/* Footer */}
        <AppFooter onPrivacy={()=>setShowPrivacy(true)} onTerms={()=>setShowTerms(true)} />
